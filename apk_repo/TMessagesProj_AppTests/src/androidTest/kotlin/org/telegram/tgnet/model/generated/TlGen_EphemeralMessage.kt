...
public sealed class TlGen_EphemeralMessage : TlGen_Object {
  public data class TL_ephemeralMessage(
    ...
    public val welcome_template: Boolean,
    ...
    internal val flags: UInt
      get() {
        ...
        if (welcome_template) result = result or 32U
        if (reply_to != null) result = result or 64U
        if (invert_media) result = result or 128U
        if (rich_message != null) result = result or 256U
        if (peer_id != null) result = result or 512U
        if (chat_instance != null) result = result or 1024U
        if (anchor_msg_id != null) result = result or 2048U
        if (noforwards) result = result or 4096U
        return result
      }
    ...
    public override fun serializeToStream(stream: OutputSerializedData) {
      ...
      anchor_msg_id?.let { stream.writeInt32(it) }
    }

    public companion object {
      public const val MAGIC: UInt = 0xDD27BEE9U
    }
  }

  public data class TL_ephemeralMessage_layer228(
    public val `out`: Boolean,
    public val id: Int,
    public val from_id: TlGen_Peer,
    public val peer_id: TlGen_Peer,
