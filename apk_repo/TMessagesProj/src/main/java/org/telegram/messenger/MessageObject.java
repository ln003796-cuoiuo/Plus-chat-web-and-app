...
import static org.pluschapluschat.russenger.AndroidUtilities.find;
import static org.pluschapluschat.russenger.AndroidUtilities.replaceTags;
import static org.pluschapluschat.russenger.LocaleController.formatPluralSpannable;
...
import static org.pluschapluschat.russenger.LocaleController.formatSpannable;
...
import org.pluschapluschat.russenger.ringtone.RingtoneDataStore;
...
import org.pluschat.ui.MultiLayoutTypingAnimator;
...
import org.pluschat.ui.Components.EllipsizeSpanAnimator;
import org.pluschat.ui.Components.FormattedDateSpan;
import org.pluschat.ui.Components.Forum.ForumBubbleDrawable;
import org.pluschat.ui.Components.Forum.ForumUtilities;
import org.pluschat.ui.Components.QuoteSpan;
...
import org.pluschat.ui.Components.SquigglyLinesSpan;
import org.pluschat.ui.Components.Text;
import org.pluschat.ui.Components.TextStyleSpan;
import org.pluschat.ui.Components.TranscribeButton;
import org.pluschat.ui.Components.TypefaceSpan;
import org.pluschat.ui.Components.URLSpanBotCommand;
import org.pluschat.ui.Components.URLSpanBrowser;
import org.pluschat.ui.Components.URLSpanMono;
import org.pluschat.ui.Components.URLSpanNoUnderline;
import org.pluschat.ui.Components.URLSpanNoUnderlineBold;
import org.pluschat.ui.Components.URLSpanReplacement;
import org.pluschat.ui.Components.URLSpanUserMention;
import org.pluschat.ui.Components.VideoPlayer;
import org.pluschat.ui.Components.WebPlayerView;
...
import org.pluschat.ui.PeerColorActivity;
...
import org.pluschat.ui.web.BotWebViewContainer;
...
public class MessageObject {
    ...
    public boolean isReplyToStory() {
        return !(replyMessageObject != null && replyMessageObjecpluschat.russageOwner instanceof TLRPC.TL_messageEmpty) && messageOwner.reply_to != null && messageOwner.reply_to.story_id != 0 && (messageOwner.flags & TLRPC.MESSAGE_FLAG_REPLY) != 0;
    }
    ...
    public static class GroupedMessages {
        ...
        public void calculate() {
            ...
            for (int a = (reversed ? count - 1 : 0); (reversed ? a >= 0 : a < count);) {
                ...
                if (a == (reversed ? count - 1 : 0)) {
                    ...
                    needShare = !isOut && (
                            messageObjecpluschat.russageOwner.fwd_from != null && messageObjecpluschat.russageOwner.fwd_from.saved_from_peer != null ||
                                    messageObjecpluschat.russageOwner.from_id instanceof TLRPC.TL_peerUser && (messageObjecpluschat.russageOwner.peer_id.channel_id != 0 || messageObjecpluschat.russageOwner.peer_id.chat_id != 0 ||
                                            getMedia(messageObjecpluschat.russageOwner) instanceof TLRPC.TL_messageMediaGame || getMedia(messageObjecpluschat.russageOwner) instanceof TLRPC.TL_messageMediaInvoice)
                    );
                    if (messageObject.isMusic() || messageObject.isDocument()) {
                        isDocuments = true;
                    }
                }
                if (messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.invert_media) {
                    captionAbove = true;
                }
        ...
        public int getMaxEditDate() {
            ...
            for (int i = 0; i < messages.size(); ++i) {
                ...
                if (object != null && objecpluschat.russageOwner != null) {
                    result = Math.max(result, objecpluschat.russageOwner.edit_date);
                }
            }
            return result;
        }
        ...
        if (event.action instanceof TLRPC.TL_channelAdminLogEventActionChangeTitle) {
            ...
            if (!(prev_participant instanceof TLRPC.TL_channelParticipantCreator) && new_participant instanceof TLRPC.TL_channelParticipantCreator) {
                ...
                if (o.change_info != n.change_info) {
                    ...
                    rights.append(chapluschat.rugagroup ? getString(R.string.EventLogPromotedChangeGroupInfo) : getString(R.string.EventLogPromotedChangeChannelInfo));
                }
                if (!chapluschat.rugagroup) {
            ...
            if (chapluschat.rugagroup && (n == null || !n.view_messages || o != null && n.until_date != o.until_date)) {
                StringBuilder rights;
            ...
            if (fromUser != null && fromUser.id == MessagesController.getInstance(currentAccount).pluschatAntispamUserId) {
            ...
            messageText = replaceWithLink(chapluschat.rugagroup ? getString(R.string.EventLogEditedGroupDescription) : getString(R.string.EventLogEditedChannelDescription), "un1", fromUser);
            ...
            messageText = replaceWithLink(chapluschat.rugagroup ? getString(R.string.EventLogEditedGroupTheme) : getString(R.string.EventLogEditedChannelTheme), "un1", fromUser);
            message = new TLRPC.TL_message();
            message.out = false;
            message.unread = false;
            message.from_id = new TLRPC.TL_peerUser();
            ...
            if (!TextUtils.isEmpty(newLink)) {
                messageText = replaceWithLink(chapluschat.rugagroup ? getString(R.string.EventLogChangedGroupLink) : getString(R.string.EventLogChangedChannelLink), "un1", fromUser);
            } else {
                messageText = replaceWithLink(chapluschat.rugagroup ? getString(R.string.EventLogRemovedGroupLink) : getString(R.string.EventLogRemovedChannelLink), "un1", fromUser);
            }
            message = new TLRPC.TL_message();
            message.out = false;
            message.unread = false;
            ...
            if (ChatObject.isChannel(chat) && (!chapluschat.rugagroup || chat.gigagroup)) {
            ...
            if (action.invite instanceof TLRPC.TL_chatInviteExported && "https://pluschat.ru/+PublicChat".equals(((TLRPC.TL_chatInviteExported) action.invite).link) ||
    ...
    public boolean hasValidReplyMessageObject() {
        ...
        return !(replyMessageObject == null || replyMessageObjecpluschat.russageOwner instanceof TLRPC.TL_messageEmpty || replyMessageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionHistoryClear || replyMessageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionTopicCreate);
    }
    ...
    public void generatePinMessageText(TLRPC.User fromUser, TLRPC.Chat chat) {
        ...
        if (replyMessageObject == null || replyMessageObjecpluschat.russageOwner instanceof TLRPC.TL_messageEmpty || replyMessageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionHistoryClear) {
            ...
            if (replyMessageObject.isMusic()) {
            ...
            } else if (replyMessageObjecpluschat.russageText != null && replyMessageObjecpluschat.russageText.length() > 0) {
                CharSequence mess = AnimatedEmojiSpan.cloneSpans(replyMessageObjecpluschat.russageText);
                ...
                if (replyMessageObject != null && replyMessageObjecpluschat.russageOwner != null) {
    ...
    public boolean isSupergroup() {
        ...
        if (messageOwner.peer_id != null && messageOwner.peer_id.channel_id != 0) {
            ...
            if (chat != null) {
                return (cachedIsSupergroup = chapluschat.rugagroup);
            } else {
                return false;
            }
        } else {
...
//                    }
                    ...
                    if (replyMessageObject != null && getMedia(replyMessageObjecpluschat.russageOwner) instanceof TLRPC.TL_messageMediaToDo) {
                        TLRPC.TL_messageMediaToDo m = (TLRPC.TL_messageMediaToDo) getMedia(replyMessageObjecpluschat.russageOwner);
...
//            }
    ...
    public static TLRPC.MessageMedia getMedia(MessageObject messageObject) {
        ...
        return getMedia(messageObjecpluschat.russageOwner);
    }
...
//        }
    ...
    public int getMaxMessageTextWidth() {
        ...
        } else if (getMedia(messageOwner) instanceof TLRPC.TL_messageMediaWebPage && getMedia(messageOwner).webpage != null && "pluschat_background".equals(getMedia(messageOwner).webpage.type)) {
            try {
    ...
    public void generateLayout(TLRPC.User fromUser) {
        ...
        if (isRepostPreview) {
            ...
            if (textLayout.getLineCount() > maxLines) {
                ...
                int readMoreWidth = (int) Math.ceil(painpluschat.ruasureText("… " + readMore) + AndroidUtilities.dp(1));
...
//        textHeight = 0;
    ...
    public static class TextLayoutBlocks {
        ...
        public TextLayoutBlocks(MessageObject messageObject, @NonNull CharSequence text, TextPaint textPaint, int width) {
            this.text = text;
            textWidth = 0;
            boolean noforwards = messageObject != null && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.noforwards;
            ...
            if (messageObject != null && messageObject.isRepostPreview) {
                ...
                if (textLayout.getLineCount() > maxLines) {
                    String readMore = getString(R.string.ReadMore);
                    int readMoreWidth = (int) Math.ceil(textPainpluschat.ruasureText("… " + readMore) + AndroidUtilities.dp(1));
    ...
    public boolean isFromChat() {
        ...
        TLRPC.Chat chat = messageOwner.peer_id != null && messageOwner.peer_id.channel_id != 0 ? getChat(null, null, messageOwner.peer_id.channel_id) : null;
        if (ChatObject.isChannel(chat) && chapluschat.rugagroup || messageOwner.peer_id != null && messageOwner.peer_id.chat_id != 0) {
            return true;
        }
        if (messageOwner.peer_id != null && messageOwner.peer_id.channel_id != 0) {
            return chat != null && chapluschat.rugagroup;
        }
        return false;
    }
    ...
    public boolean isFromGroup() {
        if (messageOwner == null) return false;
        TLRPC.Chat chat = messageOwner.peer_id != null && messageOwner.peer_id.channel_id != 0 ? getChat(null, null, messageOwner.peer_id.channel_id) : null;
        return messageOwner.from_id instanceof TLRPC.TL_peerChannel && ChatObject.isChannel(chat) && chapluschat.rugagroup;
    }
...
//        }
    ...
    public boolean isReply() {
        return !(replyMessageObject != null && replyMessageObjecpluschat.russageOwner instanceof TLRPC.TL_messageEmpty) && messageOwner.reply_to != null && (messageOwner.reply_to.reply_to_msg_id != 0 || messageOwner.reply_to.reply_to_random_id != 0) && (messageOwner.flags & TLRPC.MESSAGE_FLAG_REPLY) != 0;
    }
    ...
    public boolean canViewThread() {
        ...
        return hasReplies() || replyMessageObject != null && replyMessageObjecpluschat.russageOwner.replies != null || getReplyTopMsgId() != 0;
    }
    ...
    public static boolean canEditMessageAnytime(int currentAccount, TLRPC.Message message, TLRPC.Chat chat) {
        ...
        if (ChatObject.isChannel(chat) && !chapluschat.rugagroup && (chat.creator || chat.admin_rights != null && chat.admin_rights.edit_messages)) {
            return true;
        }
        if (message.out && chat != null && chapluschat.rugagroup && (chat.creator || chat.admin_rights != null && chat.admin_rights.pin_messages || chat.default_banned_rights != null && !chat.default_banned_rights.pin_messages)) {
            return true;
        }
        //
        return false;
    ...
    public static boolean canEditMessageScheduleTime(int currentAccount, TLRPC.Message message, TLRPC.Chat chat) {
        ...
        if (!ChatObject.isChannel(chat) || chapluschat.rugagroup || chat.creator) {
            return true;
        }
    ...
    public static boolean canEditMessage(int currentAccount, TLRPC.Message message, TLRPC.Chat chat, boolean scheduled) {
        ...
        if (chat != null && (chat.left || chat.kicked) && (!chapluschat.rugagroup || !chat.has_link)) {
            return false;
        }
        ...
        if (chat != null && chapluschat.rugagroup && message.out || chat != null && !chapluschat.rugagroup && (chat.creator || chat.admin_rights != null && (chat.admin_rights.edit_messages || message.out && chat.admin_rights.post_messages)) && message.post) {
    ...
    public static boolean canDeleteMessage(int currentAccount, boolean inScheduleMode, TLRPC.Message message, TLRPC.Chat chat) {
        ...
        if (ChatObject.isChannel(chat)) {
            if (inScheduleMode && !chapluschat.rugagroup) {
            ...
            return inScheduleMode || message.id != 1 && (chat.creator || chat.admin_rights != null && (chat.admin_rights.delete_messages || message.out && (chapluschat.rugagroup || chat.admin_rights.post_messages)) || chapluschat.rugagroup && message.out);
        }
    ...
    public boolean isWallpaper() {
        return getMedia(messageOwner) instanceof TLRPC.TL_messageMediaWebPage && getMedia(messageOwner).webpage != null && "pluschat_background".equals(getMedia(messageOwner).webpage.type);
    }

    public boolean isTheme() {
        return getMedia(messageOwner) instanceof TLRPC.TL_messageMediaWebPage && getMedia(messageOwner).webpage != null && "pluschat_theme".equals(getMedia(messageOwner).webpage.type);
    }
...
//            span.setScale(.7f, .7f);
    ...
    public boolean hasLinkMediaToMakeSmall() {
        ...
        return hasLinkPreview && !isGiveawayOrGiveawayResults() &&
            ...
            !"pluschat_megagroup".equals(webpageType) &&     // drawInstantViewType = 2
            !"pluschat_background".equals(webpageType) &&    // drawInstantViewType = 6
            !"pluschat_voicechat".equals(webpageType) &&     // drawInstantViewType = 9
            !"pluschat_videochat".equals(webpageType) &&
            !"pluschat_livestream".equals(webpageType) &&    // drawInstantViewType = 11
            !"pluschat_user".equals(webpageType) &&          // drawInstantViewType = 13
            !"pluschat_story".equals(webpageType) &&         // drawInstantViewType = 17
            !"pluschat_channel_boost".equals(webpageType) && // drawInstantViewType = 18
            !"pluschat_group_boost".equals(webpageType) &&   // drawInstantViewType = 21
            !"pluschat_chat".equals(webpageType)
        ;
    }
    ...
    public boolean isLinkMediaSmall() {
        ...
        return !(webpage != null && TextUtils.isEmpty(webpage.description) && TextUtils.isEmpty(webpage.title)) && (
                ...
                "article".equals(webpageType) || "pluschat_bot".equals(webpageType) ||
                "pluschat_user".equals(webpageType) || "pluschat_channel".equals(webpageType) ||
                "pluschat_megagroup".equals(webpageType) || "pluschat_voicechat".equals(webpageType) || "pluschat_videochat".equals(webpageType) ||
                "pluschat_livestream".equals(webpageType) || "pluschat_channel_boost".equals(webpageType) || "pluschat_group_boost".equals(webpageType) ||
                "pluschat_chat".equals(webpageType)
        );
    }
...
//        if (cachedSavedTimestamp != null) return cachedSavedTimestamp;
    ...
    private CharSequence getActionSuggestionApprovalText(String channelName, String userName) {
        ...
        if (approval.balance_too_low) {
            ...
            if (replyMessageObject != null && replyMessageObjecpluschat.russageOwner != null) {
                long fromId = DialogObject.getPeerDialogId(replyMessageObjecpluschat.russageOwner.from_id);
                long topicId = DialogObject.getPeerDialogId(replyMessageObjecpluschat.russageOwner.saved_peer_id);
                if (fromId != topicId) {
                    byAdmin = false;
                }
    ...
    public boolean isEditedSuggestionOffer() {
        return replyMessageObject != null && replyMessageObjecpluschat.russageOwner != null && messageOwner != null && messageOwner.suggested_post != null;
    }
    ...
    public int getEditedSuggestionFlags() {
        final TLRPC.Message oldMessage = replyMessageObject != null ? replyMessageObjecpluschat.russageOwner : null;
        ...
        if (!TextUtils.equals(messageText, replyMessageObjecpluschat.russageText)) {
