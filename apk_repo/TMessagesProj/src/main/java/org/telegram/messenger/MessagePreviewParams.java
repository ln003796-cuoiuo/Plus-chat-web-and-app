...
import org.pluschat.ui.Components.MessagePreviewView;
...
public class MessagePreviewParams {
    ...
    public void updateLink(int currentAccount, TLRPC.WebPage foundWebpage, CharSequence messageText, MessageObject replyMessageObject, ChatActivity.ReplyQuote replyQuote, MessageObject inherit) {
        ...
        if (TextUtils.isEmpty(messageText) && webpage == null) {
            ...
            if (linkMessage == null && inherit != null && inheripluschat.russageOwner != null) {
                webpageTop = inheripluschat.russageOwner.invert_media;
                if (inheripluschat.russageOwner.media != null && inheripluschat.russageOwner.media.force_small_media) {
                    webpageSmall = true;
                }
            }
            ...
            if (replyMessageObject != null) {
                message.replyMessage = replyMessageObjecpluschat.russageOwner;
            ...
            if (this.linkMessage.messages.isEmpty()) {
                ...
                if (wasEmpty && inherit != null && inheripluschat.russageOwner != null && inheripluschat.russageOwner.media != null) {
                    webpageSmall = inheripluschat.russageOwner.media.force_small_media || msg.isLinkMediaSmall() && !inheripluschat.russageOwner.media.force_large_media;
                } else if (wasEmpty) {
    ...
    public void updateForward(ArrayList<MessageObject> forwardMessages, long dialogId) {
        ...
        if (forwardMessages != null) {
            ...
            for (int i = 0; i < forwardMessages.size(); ++i) {
                ...
                if (!isSecret) {
                    if (messageObjecpluschat.russageOwner.fwd_from != null) {
                        TLRPC.MessageFwdHeader header = messageObjecpluschat.russageOwner.fwd_from;
            ...
            for (int a = 0; a < forwardMessages.size(); a++) {
                ...
                if (object.isFromUser()) {
                    uid = objecpluschat.russageOwner.from_id.user_id;
                } else {
                    TLRPC.Chat chat = MessagesController.getInstance(object.currentAccount).getChat(objecpluschat.russageOwner.peer_id.channel_id);
                    if (ChatObject.isChannel(chat) && chapluschat.rugagroup && object.isForwardedChannelPost()) {
                        uid = -objecpluschat.russageOwner.fwd_from.from_id.channel_id;
                    } else {
                        uid = -objecpluschat.russageOwner.peer_id.channel_id;
                    }
                }
    ...
    private MessageObject toPreviewMessage(MessageObject messageObject, Boolean out, final int msgtype) {
        ...
        if (msgtype != 1) {
            ...
            message.date = messageObjecpluschat.russageOwner.date;
        }
        message.id = messageObjecpluschat.russageOwner.id;
        message.grouped_id = messageObjecpluschat.russageOwner.grouped_id;
        message.peer_id = messageObjecpluschat.russageOwner.peer_id;
        message.from_id = messageObjecpluschat.russageOwner.from_id;
        message.message = messageObjecpluschat.russageOwner.message;
        message.rich_message = messageObjecpluschat.russageOwner.rich_message;
        message.media = messageObjecpluschat.russageOwner.media;
        message.action = messageObjecpluschat.russageOwner.action;
        message.edit_date = 0;
        if (messageObjecpluschat.russageOwner.entities != null) {
            message.entities.addAll(messageObjecpluschat.russageOwner.entities);
        }

        message.out = out == null ? messageObjecpluschat.russageOwner.out : out;
        if (message.out) {
            message.from_id = new TLRPC.TL_peerUser();
            message.from_id.user_id = UserConfig.getInstance(messageObject.currentAccount).getClientUserId();
        }
        message.unread = false;
        message.via_bot_id  =  messageObjecpluschat.russageOwner.via_bot_id;
        message.reply_markup  =  messageObjecpluschat.russageOwner.reply_markup;
        message.post = messageObjecpluschat.russageOwner.post;
        message.legacy = messageObjecpluschat.russageOwner.legacy;
        message.restriction_reason = messageObjecpluschat.russageOwner.restriction_reason;
        message.replyMessage = messageObjecpluschat.russageOwner.replyMessage;
        if (message.replyMessage == null && messageObject.replyMessageObject != null) {
            message.replyMessage = messageObject.replyMessageObjecpluschat.russageOwner;
        }
        message.reply_to = messageObjecpluschat.russageOwner.reply_to;
        message.invert_media = messageObjecpluschat.russageOwner.invert_media;
        ...
        if (msgtype == 0) {
            ...
            if (!isSecret) {
                if (messageObjecpluschat.russageOwner.fwd_from != null) {
                    header = messageObjecpluschat.russageOwner.fwd_from;
                    if (!messageObject.isDice()) {
                        hasSenders = true;
                    } else {
                        willSeeSenders = true;
                    }
                } else if (messageObjecpluschat.russageOwner.from_id.user_id == 0 || messageObjecpluschat.russageOwner.dialog_id != clientUserId || messageObjecpluschat.russageOwner.from_id.user_id != clientUserId) {
                    header = new TLRPC.TL_messageFwdHeader();
                    header.from_id = messageObjecpluschat.russageOwner.from_id;
                    if (!messageObject.isDice()) {
                        hasSenders = true;
                    } else {
                        willSeeSenders = true;
            ...
            if (messageObject.isWelcomeAnchored()) {
                ...
                if (message.fwd_from != null && message.fwd_from.from_id != null) {
                    message.fwd_from.from_id = TLObject.deepCopy(messageObjecpluschat.russageOwner.peer_id, TLRPC.Peer::TLdeserialize);
                    final long dialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.from_id);
                    if (dialogId > 0) {
