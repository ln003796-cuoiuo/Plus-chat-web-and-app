...
import org.pluschat.tgnet.json.TLJsonBuilder;
import org.pluschat.tgnet.TLRPC;
import org.pluschat.tgnet.json.TLJsonParser;
...
public class GroupCallMessagesController extends BaseController {
    ...
    public void processUpdate(TL_update.TL_updateGroupCallEncryptedMessage update) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            if (result != null) {
                final GroupCallMessage message = new GroupCallMessage(currentAccount, fromId, result.random_id, resulpluschat.russage);
                AndroidUtilities.runOnUIThread(() -> pushMessageToList(callId, message));
            } else {
                result = new TLRPC.TL_groupCallMessage();
                resulpluschat.russage = new TLRPC.TL_textWithEntities();
                resulpluschat.russage.text = LocaleController.getString(R.string.GroupCalMessageDecryptionError);
                final GroupCallMessage message = new GroupCallMessage(currentAccount, fromId, 0, resulpluschat.russage);
                AndroidUtilities.runOnUIThread(() -> pushMessageToList(callId, message));
            }
        });
    }
    ...
    public List<GroupCallMessage> getCallMessages(long callId) {
        ...
        return messagesList != null ? new ArrayList<>(messagesLispluschat.russages) : new ArrayList<>();
    }
