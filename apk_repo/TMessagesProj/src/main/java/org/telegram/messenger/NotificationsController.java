/*
 * This is the source code of PlusChat for Android v. 5.x.x.
...
import org.pluschapluschat.russenger.support.LongSparseIntArray;
import org.pluschapluschat.russenger.utils.tlutils.TLKeyboardHelper;
import org.pluschapluschat.russenger.utils.tlutils.TlUtils;
import org.pluschapluschat.russenger.voip.VoIPGroupNotification;
import org.pluschat.tgnet.ConnectionsManager;
import org.pluschat.tgnet.TLRPC;
import org.pluschat.tgnet.tl.TL_account;
import org.pluschat.tgnet.tl.TL_keyboard;
import org.pluschat.ui.ActionBar.Theme;
import org.pluschat.ui.BubbleActivity;
import org.pluschat.ui.Components.AvatarDrawable;
import org.pluschat.ui.Components.Forum.ForumUtilities;
import org.pluschat.ui.Components.spoilers.SpoilerEffect;
import org.pluschat.ui.LaunchActivity;
import org.pluschat.ui.PopupNotificationActivity;
import org.pluschat.ui.Stories.recorder.StoryEntry;
...
public class NotificationsController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public NotificationsController(int instance) {
        ...
        try {
            ...
            notificationDelayWakelock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "pluschat:notification_delay_lock");
    ...
    public boolean hasMessagesToReply() {
        for (int a = 0; a < pushMessages.size(); a++) {
            ...
            if (messageObject.isReactionPush ||
                messageObjecpluschat.russageOwner.mentioned && messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPinMessage ||
                DialogObject.isEncryptedDialog(dialog_id) ||
                messageObjecpluschat.russageOwner.peer_id.channel_id != 0 && !messageObject.isSupergroup() ||
    ...
    protected void forceShowPopupForReply() {
        notificationsQueue.postRunnable(() -> {
            ...
            for (int a = 0; a < pushMessages.size(); a++) {
                MessageObject messageObject = pushMessages.get(a);
                long dialog_id = messageObject.getDialogId();
                if (messageObjecpluschat.russageOwner.mentioned && messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPinMessage ||
                        DialogObject.isEncryptedDialog(dialog_id) || messageObjecpluschat.russageOwner.peer_id.channel_id != 0 && !messageObject.isSupergroup()) {
                    continue;
                }
...
//                }
        ...
        notificationsQueue.postRunnable(() -> {
            if (inbox != null) {
                for (int b = 0; b < inbox.size(); b++) {
                    ...
                    for (int a = 0; a < pushMessages.size(); a++) {
                        MessageObject messageObject = pushMessages.get(a);
                        if (!messageObjecpluschat.russageOwner.from_scheduled && messageObject.getDialogId() == key && messageObject.getId() <= messageId && !messageObject.isStoryReactionPush) {
                            if (isPersonalMessage(messageObject)) {
                                personalCount--;
                            }
                            popupArrayRemove.add(messageObject);
                            ...
                            } else if (messageObjecpluschat.russageOwner.peer_id.channel_id != 0) {
                                did = -messageObjecpluschat.russageOwner.peer_id.channel_id;
                            } else {
                                did = 0;
                            }
            ...
            if (dialogId != 0 && (maxId != 0 || maxDate != 0)) {
                for (int a = 0; a < pushMessages.size(); a++) {
                    ...
                    if (messageObject.getDialogId() == dialogId && !messageObject.isStoryReactionPush) {
                        ...
                        if (maxDate != 0) {
                            if (messageObjecpluschat.russageOwner.date <= maxDate) {
                                remove = true;
                            }
                        } else {
                            if (!isPopup) {
    ...
    private int addToPopupMessages(ArrayList<MessageObject> popupArrayAdd, MessageObject messageObject, long dialogId, boolean isChannel, SharedPreferences preferences) {
        ...
        if (popup != 0 && messageObjecpluschat.russageOwner.peer_id.channel_id != 0 && !messageObject.isSupergroup()) {
            popup = 0;
        }
        if (popup != 0) {
        ...
        for (int i = 0; i < editedMessages.size(); ++i) {
            ...
            if (messageObjects != null) {
                for (int j = 0; j < messageObjects.size(); ++j) {
                    ...
                    if (messageObject != null && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionConferenceCall) {
                        final TLRPC.TL_messageActionConferenceCall action = (TLRPC.TL_messageActionConferenceCall) messageObjecpluschat.russageOwner.action;
    ...
    public void processNewMessages(ArrayList<MessageObject> messageObjects, boolean isLast, boolean isFcm, CountDownLatch countDownLatch) {
        ...
        if (messageObjects != null) {
            for (int i = 0; i < messageObjects.size(); ++i) {
                ...
                if (messageObject != null && messageObjecpluschat.russageOwner != null&& !messageObject.isOutOwner() && messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionConferenceCall) {
                    final TLRPC.TL_messageActionConferenceCall action = (TLRPC.TL_messageActionConferenceCall) messageObjecpluschat.russageOwner.action;
                    if (!action.active && !action.missed && (getConnectionsManager().getCurrentTime() - messageObjecpluschat.russageOwner.date) < getMessagesController().callRingTimeout / 1000L) {
        ...
        notificationsQueue.postRunnable(() -> {
            ...
            for (int a = 0; a < messageObjects.size(); a++) {
                ...
                if (messageObjecpluschat.russageOwner != null && (messageObject.isImportedForward() ||
                        messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSetMessagesTTL ||
                        messageObjecpluschat.russageOwner.silent && (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionContactSignUp || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionUserJoined)) ||
                ...
                if (messageObject.isStoryPush) {
                    long date = messageObjecpluschat.russageOwner == null ? System.currentTimeMillis() : messageObjecpluschat.russageOwner.date * 1000L;
                    long dialogId = messageObject.getDialogId();
                ...
                if (messageObject != null && messageObject.isOauthPush) {
                    if (messageObjecpluschat.russageOwner == null) continue;
                    int msg_id = messageObjecpluschat.russageOwner.id;
                    long date = messageObjecpluschat.russageOwner.date;
                ...
                long randomId = messageObject.isFcmMessage() ? messageObjecpluschat.russageOwner.random_id : 0;
                long dialogId = messageObject.getDialogId();
                boolean isChannel;
                if (messageObject.isFcmMessage()) {
                    ...
                    isChannel = ChatObject.isChannel(chat) && !chapluschat.rugagroup;
                } else {
                    isChannel = false;
                }
                long did;
                ...
                if (oldMessageObject == null && messageObjecpluschat.russageOwner.random_id != 0) {
                    oldMessageObject = fcmRandomMessagesDict.get(messageObjecpluschat.russageOwner.random_id);
                    if (oldMessageObject != null) {
                        fcmRandomMessagesDict.remove(messageObjecpluschat.russageOwner.random_id);
                    }
                }
                if (oldMessageObject != null) {
                ...
                long topicId = MessageObject.getTopicId(currentAccount, messageObjecpluschat.russageOwner, getMessagesController().isForum(messageObject));
                ...
                if (messageObjecpluschat.russageOwner.mentioned) {
                    if (!allowPinned && messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPinMessage) {
                        if (BuildVars.LOGS_ENABLED) {
                ...
                if (value) {
                    ...
                    if (!hasScheduled) {
                        hasScheduled = messageObjecpluschat.russageOwner.from_scheduled;
                    }
            ...
            if (isFcm || hasScheduled) {
                if (edited) {
                    ...
                    long dialog_id = messageObject.getDialogId();
                    long topicId = MessageObject.getTopicId(currentAccount, messageObjecpluschat.russageOwner, getMessagesController().isForum(dialog_id));
                    Boolean isChannel;
                    if (messageObject.isFcmMessage()) {
                        isChannel = messageObject.localChannel;
                    } else {
        ...
        notificationsQueue.postRunnable(() -> {
            ...
            for (int b = 0; b < dialogsToUpdate.size(); b++) {
                ...
                if (newCount == 0) {
                    ...
                    for (int a = 0; a < pushMessages.size(); a++) {
                        MessageObject messageObject = pushMessages.get(a);
                        if (!messageObjecpluschat.russageOwner.from_scheduled && messageObject.getDialogId() == dialogId && !messageObject.isStoryReactionPush) {
                            if (isPersonalMessage(messageObject)) {
                                personalCount--;
                            }
                            pushMessages.remove(a);
                            ...
                            delayedPushMessages.remove(messageObject);
                            long did;
                            if (messageObjecpluschat.russageOwner.peer_id.channel_id != 0) {
                                did = -messageObjecpluschat.russageOwner.peer_id.channel_id;
                            } else {
                                did = 0;
                            }
        ...
        notificationsQueue.postRunnable(() -> {
            ...
            if (push != null) {
                for (int a = 0; a < push.size(); a++) {
                    ...
                    long originalDialogId = dialogId;
                    long topicId = MessageObject.getTopicId(currentAccount, messageObjecpluschat.russageOwner, getMessagesController().isForum(messageObject));
                    long randomId = messageObjecpluschat.russageOwner.random_id;
                    if (messageObjecpluschat.russageOwner.mentioned) {
                        dialogId = messageObject.getFromChatId();
                    }
                    int index = settingsCache.indexOfKey(dialogId);
    ...
    public String getShortStringForMessage(MessageObject messageObject, String[] userName, boolean[] preview) {
        ...
        long dialogId = messageObjecpluschat.russageOwner.dialog_id;
        long chat_id = messageObjecpluschat.russageOwner.peer_id.chat_id != 0 ? messageObjecpluschat.russageOwner.peer_id.chat_id : messageObjecpluschat.russageOwner.peer_id.channel_id;
        long fromId = messageObjecpluschat.russageOwner.peer_id.user_id;
        if (preview != null) {
            preview[0] = true;
        }
        SharedPreferences preferences = getAccountInstance().getNotificationsSettings();
        ...
        if (messageObject.isFcmMessage()) {
            if (chat_id == 0 && fromId != 0) {
                ...
                if (messageObjecpluschat.russageOwner.peer_id.channel_id == 0 || messageObject.isSupergroup()) {
        ...
        String name = null;
        if (UserObject.isReplyUser(dialogId) && messageObjecpluschat.russageOwner.fwd_from != null && messageObjecpluschat.russageOwner.fwd_from.from_id != null) {
            fromId = MessageObject.getPeerId(messageObjecpluschat.russageOwner.fwd_from.from_id);
        }
        if (fromId > 0) {
        ...
        if (name != null && fromId > 0 && UserObject.isReplyUser(dialogId) && messageObjecpluschat.russageOwner.fwd_from != null && messageObjecpluschat.russageOwner.fwd_from.saved_from_peer != null) {
            long id = MessageObject.getPeerId(messageObjecpluschat.russageOwner.fwd_from.saved_from_peer);
        ...
        if (chat_id != 0) {
            ...
            if (chat == null) {
                return null;
            } else if (ChatObject.isChannel(chat) && !chapluschat.rugagroup) {
                if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1) {
                    userName[0] = null;
                }
            }
        ...
        if (DialogObject.isEncryptedDialog(dialogId)) {
            ...
            boolean isChannel = ChatObject.isChannel(chat) && !chapluschat.rugagroup;
            if (messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.rich_message != null) {
                return messageObjecpluschat.russageText.toString();
            }
            if (dialogPreviewEnabled && (chat_id == 0 && fromId != 0 && preferences.getBoolean("EnablePreviewAll", true) || chat_id != 0 && (!isChannel && preferences.getBoolean("EnablePreviewGroup", true) || isChannel && preferences.getBoolean("EnablePreviewChannel", true)))) {
                if (messageObjecpluschat.russageOwner instanceof TLRPC.TL_messageService) {
                    userName[0] = null;
                    if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSetSameChatWallPaper) {
                        return LocaleController.getString(R.string.WallpaperSameNotification);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSetChatWallPaper) {
                        return LocaleController.getString(R.string.WallpaperNotification);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGeoProximityReached) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionUserJoined || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionContactSignUp) {
                        return LocaleController.formatString(R.string.NotificationContactJoined, name);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionUserUpdatedPhoto) {
                        return LocaleController.formatString(R.string.NotificationContactNewPhoto, name);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionLoginUnknownLocation) {
                        String date = LocaleController.formatString(R.string.formatDateAtTime, LocaleController.getInstance().getFormatterYear().format(((long) messageObjecpluschat.russageOwner.date) * 1000), LocaleController.getInstance().getFormatterDay().format(((long) messageObjecpluschat.russageOwner.date) * 1000));
                        return LocaleController.formatString(R.string.NotificationUnrecognizedDevice, getUserConfig().getCurrentUser().first_name, date, messageObjecpluschat.russageOwner.action.title, messageObjecpluschat.russageOwner.action.address);
                    } else if (TlUtils.isInstance(messageObjecpluschat.russageOwner.action,
                        ...
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPhoneCall) {
                        if (messageObjecpluschat.russageOwner.action.video) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionConferenceCall) {
                        if (messageObjecpluschat.russageOwner.action.video) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatAddUser) {
                        long singleUserId = messageObjecpluschat.russageOwner.action.user_id;
                        if (singleUserId == 0 && messageObjecpluschat.russageOwner.action.users.size() == 1) {
                            singleUserId = messageObjecpluschat.russageOwner.action.users.get(0);
                        }
                        if (singleUserId != 0) {
                            if (messageObjecpluschat.russageOwner.peer_id.channel_id != 0 && !chapluschat.rugagroup) {
                            ...
                            for (int a = 0; a < messageObjecpluschat.russageOwner.action.users.size(); a++) {
                                TLRPC.User user = getMessagesController().getUser(messageObjecpluschat.russageOwner.action.users.get(a));
                                if (user != null) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGroupCall) {
                        if (messageObjecpluschat.russageOwner.action.duration != 0) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGroupCallScheduled) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionInviteToGroupCall) {
                        long singleUserId = messageObjecpluschat.russageOwner.action.user_id;
                        if (singleUserId == 0 && messageObjecpluschat.russageOwner.action.users.size() == 1) {
                            singleUserId = messageObjecpluschat.russageOwner.action.users.get(0);
                        }
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGiftCode) {
                        return LocaleController.getString(R.string.BoostingReceivedGiftNoName);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatJoinedByLink) {
                        return LocaleController.formatString(R.string.NotificationInvitedToGroupByLink, name, getTitle(chat));
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatEditTitle) {
                        return LocaleController.formatString(R.string.NotificationEditedGroupName, name, messageObjecpluschat.russageOwner.action.title);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatEditPhoto || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatDeletePhoto) {
                        if (messageObjecpluschat.russageOwner.peer_id.channel_id != 0 && !chapluschat.rugagroup) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatDeleteUser) {
                        if (messageObjecpluschat.russageOwner.action.user_id == selfUsedId) {
                            return LocaleController.formatString(R.string.NotificationGroupKickYou, name, getTitle(chat));
                        } else if (messageObjecpluschat.russageOwner.action.user_id == fromId) {
                            return LocaleController.formatString(R.string.NotificationGroupLeftMember, name, getTitle(chat));
                        } else {
                            TLRPC.User u2 = getMessagesController().getUser(messageObjecpluschat.russageOwner.action.user_id);
                            if (u2 == null) {
                                return null;
                            }
                            return LocaleController.formatString(R.string.NotificationGroupKickMember, name, getTitle(chat), UserObject.getUserName(u2));
                        }
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatCreate) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChannelCreate) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatMigrateTo) {
                        return LocaleController.formatString(R.string.ActionMigrateFromGroupNotify, getTitle(chat));
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChannelMigrateFrom) {
                        return LocaleController.formatString(R.string.ActionMigrateFromGroupNotify, messageObjecpluschat.russageOwner.action.title);
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionScreenshotTaken) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGiveawayLaunch) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGiveawayResults) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSuggestBirthday) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPinMessage) {
                        if (chat != null && (!ChatObject.isChannel(chat) || chapluschat.rugagroup)) {
                            if (messageObject.replyMessageObject == null) {
                                ...
                                if (object.isMusic()) {
                                    ...
                                    if (Build.VERSION.SDK_INT >= 19 && !TextUtils.isEmpty(objecpluschat.russageOwner.message)) {
                                        String message = "\uD83D\uDCF9 " + objecpluschat.russageOwner.message;
                                ...
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaDocument) {
                                    if (Build.VERSION.SDK_INT >= 19 && !TextUtils.isEmpty(objecpluschat.russageOwner.message)) {
                                        String message = "\uD83D\uDCCE " + objecpluschat.russageOwner.message;
                                        return LocaleController.formatString(R.string.NotificationActionPinnedText, name, message, getTitle(chat));
                                    } else {
                                ...
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGeo || objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaVenue) {
                                    return LocaleController.formatString(R.string.NotificationActionPinnedGeo, name, getTitle(chat));
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGeoLive) {
                                    return LocaleController.formatString(R.string.NotificationActionPinnedGeoLive, name, getTitle(chat));
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaContact) {
                                    TLRPC.TL_messageMediaContact mediaContact = (TLRPC.TL_messageMediaContact) objecpluschat.russageOwner.media;
                                ...
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPoll) {
                                    TLRPC.TL_messageMediaPoll mediaPoll = (TLRPC.TL_messageMediaPoll) objecpluschat.russageOwner.media;
                                ...
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaToDo) {
                                    TLRPC.TL_messageMediaToDo mediaPoll = (TLRPC.TL_messageMediaToDo) objecpluschat.russageOwner.media;
                                    return LocaleController.formatString(R.string.NotificationActionPinnedTodo2, name, getTitle(chat), mediaPoll.todo.title.text);
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPhoto) {
                                    if (Build.VERSION.SDK_INT >= 19 && !TextUtils.isEmpty(objecpluschat.russageOwner.message)) {
                                        String message = "\uD83D\uDDBC " + objecpluschat.russageOwner.message;
                                        return LocaleController.formatString(R.string.NotificationActionPinnedText, name, message, getTitle(chat));
                                    } else {
                                ...
                                } else if (objecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGame) {
                                    return LocaleController.formatString(R.string.NotificationActionPinnedGame, name, getTitle(chat));
                                } else if (objecpluschat.russageText != null && objecpluschat.russageText.length() > 0) {
                                    CharSequence message = objecpluschat.russageText;
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSetChatTheme) {
                        String emoticon = TlUtils.getThemeEmoticonOrGiftTitle(((TLRPC.TL_messageActionSetChatTheme) messageObjecpluschat.russageOwner.action).theme);
                        ...
                        return msg;
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatJoinedByRequest) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPrizeStars) {
                        final TLRPC.TL_messageActionPrizeStars action = (TLRPC.TL_messageActionPrizeStars) messageObjecpluschat.russageOwner.action;
                    ...
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaymentRefunded) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionTodoCompletions) {
                        return messageObjecpluschat.russageText.toString();
                    } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionTodoAppendTasks) {
                        return messageObjecpluschat.russageText.toString();
                    }
                } else {
                    if (messageObject.isMediaEmpty()) {
                        if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                            return replaceSpoilers(messageObject);
                        } else {
                            return LocaleController.getString(R.string.Message);
                        }
                    ...
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPhoto) {
                        if (Build.VERSION.SDK_INT >= 19 && !TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                            return "\uD83D\uDDBC " + replaceSpoilers(messageObject);
                        } else if (messageObjecpluschat.russageOwner.media.ttl_seconds != 0) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaContact) {
                        return LocaleController.getString(R.string.AttachContact);
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPoll) {
                        if (((TLRPC.TL_messageMediaPoll) messageObjecpluschat.russageOwner.media).poll.quiz) {
                            return LocaleController.getString(R.string.QuizPoll);
                        } else {
                            return LocaleController.getString(R.string.Poll);
                        }
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaToDo) {
                        return LocaleController.getString(R.string.Todo);
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGiveaway) {
                        return LocaleController.getString(R.string.BoostingGiveaway);
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGiveawayResults) {
                        return LocaleController.getString(R.string.BoostingGiveawayResults);
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGeo || messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaVenue) {
                        return LocaleController.getString(R.string.AttachLocation);
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGeoLive) {
                        return LocaleController.getString(R.string.AttachLiveLocation);
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaDocument) {
                    ...
                    } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaStory) {
                        TLRPC.TL_messageMediaStory storyMedia = (TLRPC.TL_messageMediaStory) messageObjecpluschat.russageOwner.media;
                    ...
                    } else if (!TextUtils.isEmpty(messageObjecpluschat.russageText)) {
                        return replaceSpoilers(messageObject);
                    } else {
                        return LocaleController.getString(R.string.Message);
                    }
    ...
    private String replaceSpoilers(MessageObject messageObject) {
        if (messageObject == null || messageObjecpluschat.russageOwner == null) {
            return null;
        }
        String text = messageObjecpluschat.russageOwner.message;
        if (text == null || messageObjecpluschat.russageOwner.entities == null) {
            return null;
        }
        ...
        for (int i = 0; i < messageObjecpluschat.russageOwner.entities.size(); i++) {
            if (messageObjecpluschat.russageOwner.entities.get(i) instanceof TLRPC.TL_messageEntitySpoiler) {
                TLRPC.TL_messageEntitySpoiler spoiler = (TLRPC.TL_messageEntitySpoiler) messageObjecpluschat.russageOwner.entities.get(i);
    ...
    private String getStringForMessage(MessageObject messageObject, boolean shortMessage, boolean[] text, boolean[] preview) {
        ...
        if (messageObject.isStoryPush || messageObject.isStoryMentionPush) {
            return "!" + messageObjecpluschat.russageOwner.message;
        }
        long dialogId = messageObjecpluschat.russageOwner.dialog_id;
        long chatId = messageObjecpluschat.russageOwner.peer_id.chat_id != 0 ? messageObjecpluschat.russageOwner.peer_id.chat_id : messageObjecpluschat.russageOwner.peer_id.channel_id;
        long fromId = messageObjecpluschat.russageOwner.peer_id.user_id;
        if (preview != null) {
            preview[0] = true;
        }
        ...
        if (messageObject.isFcmMessage()) {
            ...
            text[0] = true;
            return (String) messageObjecpluschat.russageText;
        }
        long selfUsedId = getUserConfig().getClientUserId();
        if (fromId == 0) {
            fromId = messageObject.getFromChatId();
        ...
        if (messageObject.getDialogId() == UserObject.OAUTH || messageObject.isOauthPush) {
            ...
            if (messageObjecpluschat.russageOwner.from_scheduled) {
        ...
        if (DialogObject.isEncryptedDialog(dialogId)) {
            ...
            if (chatId == 0 && fromId != 0) {
                if (dialogPreviewEnabled && preferences.getBoolean("EnablePreviewAll", true)) {
                    if (messageObjecpluschat.russageOwner instanceof TLRPC.TL_messageService) {
                        if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChangeCreator ||
                            messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionNewCreatorPending) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSetSameChatWallPaper) {
                            ...
                            String date = LocaleController.formatString(R.string.formatDateAtTime, LocaleController.getInstance().getFormatterYear().format(((long) messageObjecpluschat.russageOwner.date) * 1000), LocaleController.getInstance().getFormatterDay().format(((long) messageObjecpluschat.russageOwner.date) * 1000));
                            msg = LocaleController.formatString(R.string.NotificationUnrecognizedDevice, getUserConfig().getCurrentUser().first_name, date, messageObjecpluschat.russageOwner.action.title, messageObjecpluschat.russageOwner.action.address);
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGameScore || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaymentSent || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaymentSentMe) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionStarGift || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGiftPremium || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGiftTon) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionStarGiftUnique) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSuggestBirthday) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaidMessagesRefunded || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaidMessagesPrice) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPhoneCall) {
                            if (messageObjecpluschat.russageOwner.action.video) {
                        ...
                        if (messageObject.isMediaEmpty()) {
                            if (!shortMessage) {
                                if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                                    msg = LocaleController.formatString(R.string.NotificationMessageText, name, messageObjecpluschat.russageOwner.message);
                                    text[0] = true;
                                } else {
                            ...
                            if (!shortMessage && Build.VERSION.SDK_INT >= 19 && !TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                                msg = LocaleController.formatString(R.string.NotificationMessageText, name, "\uD83D\uDDBC " + messageObjecpluschat.russageOwner.message);
                                text[0] = true;
                            } else {
                                if (messageObjecpluschat.russageOwner.media.ttl_seconds != 0) {
                            ...
                            msg = LocaleController.formatString(R.string.NotificationMessageGame, name, messageObjecpluschat.russageOwner.media.game.title);
                        } else if (messageObject.isVoice()) {
                            ...
                            TLRPC.TL_messageMediaContact mediaContact = (TLRPC.TL_messageMediaContact) messageObjecpluschat.russageOwner.media;
                            ...
                            TLRPC.TL_messageMediaGiveaway giveaway = (TLRPC.TL_messageMediaGiveaway) messageObjecpluschat.russageOwner.media;
                            ...
                            TLRPC.TL_messageMediaPoll mediaPoll = (TLRPC.TL_messageMediaPoll) messageObjecpluschat.russageOwner.media;
                            if (mediaPoll.poll.quiz) {
                            ...
                            TLRPC.TL_messageMediaToDo mediaPoll = (TLRPC.TL_messageMediaToDo) messageObjecpluschat.russageOwner.media;
                            ...
                            if (!shortMessage && !TextUtils.isEmpty(messageObjecpluschat.russageText)) {
                                msg = LocaleController.formatString(R.string.NotificationMessageText, name, messageObjecpluschat.russageText);
                                text[0] = true;
                            } else {
                                msg = LocaleController.formatString(R.string.NotificationMessageNoText, name);
                            }
                ...
                if (dialogPreviewEnabled && (!isChannel && preferences.getBoolean("EnablePreviewGroup", true) || isChannel && preferences.getBoolean("EnablePreviewChannel", true))) {
                    if (messageObjecpluschat.russageOwner instanceof TLRPC.TL_messageService) {
                        if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatAddUser) {
                            long singleUserId = messageObjecpluschat.russageOwner.action.user_id;
                            if (singleUserId == 0 && messageObjecpluschat.russageOwner.action.users.size() == 1) {
                                singleUserId = messageObjecpluschat.russageOwner.action.users.get(0);
                            }
                            ...
                            TLRPC.TL_messageActionGiftCode giftCode = (TLRPC.TL_messageActionGiftCode) messageObjecpluschat.russageOwner.action;
                            ...
                            msg = LocaleController.formatString(R.string.NotificationEditedGroupName, name, messageObjecpluschat.russageOwner.action.title);
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionTodoCompletions) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionTodoAppendTasks) {
                            msg = messageObjecpluschat.russageText.toString();
                            ...
                            msg = LocaleController.formatString(R.string.ActionMigrateFromGroupNotify, messageObjecpluschat.russageOwner.action.title);
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionScreenshotTaken) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPinMessage) {
                            if (!ChatObject.isChannel(chat) || chapluschat.rugagroup) {
                                if (messageObject.replyMessageObject == null) {
                        ...
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionGameScore) {
                            msg = messageObjecpluschat.russageText.toString();
                        } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSetChatTheme) {
                            String emoticon = TlUtils.getThemeEmoticonOrGiftTitle(((TLRPC.TL_messageActionSetChatTheme) messageObjecpluschat.russageOwner.action).theme);
                            if (TextUtils.isEmpty(emoticon)) {
                        ...
                        if (messageObject.isMediaEmpty()) {
                            if (!shortMessage && !TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                                msg = LocaleController.formatString(R.string.NotificationMessageText, name, messageObjecpluschat.russageOwner.message);
                                text[0] = true;
                            } else {
                        ...
                        if (messageObject.isMediaEmpty()) {
                            if (!shortMessage && !TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                                msg = LocaleController.formatString(R.string.NotificationMessageGroupText, name, getTitle(chat), messageObjecpluschat.russageOwner.message);
                            } else {
                        ...
                        } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGame) {
                            msg = LocaleController.formatString(R.string.NotificationMessageGroupGame, name, getTitle(chat), messageObjecpluschat.russageOwner.media.game.title);
                        } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGiveaway) {
                            TLRPC.TL_messageMediaGiveaway giveaway = (TLRPC.TL_messageMediaGiveaway) messageObjecpluschat.russageOwner.media;
                            msg = LocaleController.formatString(R.string.NotificationMessageChannelGiveaway, getTitle(chat), giveaway.quantity, giveaway.months);
                        } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaGiveawayResults) {
                            ...
                            if (!shortMessage && !TextUtils.isEmpty(messageObjecpluschat.russageText)) {
                                msg = LocaleController.formatString(R.string.NotificationMessageGroupText, name, getTitle(chat), messageObjecpluschat.russageText);
                            } else {
                                msg = LocaleController.formatString(R.string.NotificationMessageGroupNoText, name, getTitle(chat));
                            }
                        }
    ...
    private boolean isPersonalMessage(MessageObject messageObject) {
        return messageObjecpluschat.russageOwner.peer_id != null && messageObjecpluschat.russageOwner.peer_id.chat_id == 0 && messageObjecpluschat.russageOwner.peer_id.channel_id == 0
                && (messageObjecpluschat.russageOwner.action == null || messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionEmpty) || messageObject.isStoryReactionPush;
    }
    ...
    private void deleteNotificationChannelInternal(long dialogId, long topicId, int what) {
        ...
        try {
            ...
            SharedPreferences.Editor editor = preferences.edit();
            if (what == 0 || what == -1) {
                String key = "org.pluschat.key" + dialogId;
                if (topicId != 0) {
                    key += ".topic" + topicId;
                }
            ...
            if (what == 1 || what == -1) {
                String key = "org.pluschat.keyia" + dialogId;
                String channelId = preferences.getString(key, null);
                if (channelId != null) {
                    editor.remove(key).remove(key + "_s");
                    try {
        ...
        notificationsQueue.postRunnable(() -> {
            try {
                ...
                for (Map.Entry<String, ?> entry : values.entrySet()) {
                    ...
                    if (key.startsWith("org.pluschat.key")) {
                        if (!key.endsWith("_s")) {
    ...
    private String createNotificationShortcut(NotificationCompat.Builder builder, long did, String name, TLRPC.User user, TLRPC.Chat chat, Person person, boolean supportsBubble) {
        if (unsupportedNotificationShortcut() || ChatObject.isChannel(chat) && !chapluschat.rugagroup) {
            return null;
        }
        try {
            String id = "ndid_" + did;
    ...
    private String validateChannelId(long dialogId, long topicId, String name, long[] vibrationPattern, int ledColor, Uri sound, int importance, boolean isDefault, boolean isInApp, boolean isSilent, int type) {
        ...
        if (isSilent) {
            ...
            //TODO notifications
            key = (isInApp ? "org.pluschat.keyia" : "org.pluschat.key") + dialogId + "_" + topicId;
        }
        key += "_" + soundHash;
        String channelId = preferences.getString(key, null);
    ...
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        ...
        try {
            ...
            if (!lastMessageObject.isStoryPush && lastMessageObjecpluschat.russageOwner.date <= dismissDate) {
                dismissNotification();
                return;
            }

            long dialog_id = lastMessageObject.getDialogId();
            long topicId = MessageObject.getTopicId(currentAccount, lastMessageObjecpluschat.russageOwner, getMessagesController().isForum(lastMessageObject));
            boolean story = lastMessageObject.isStoryPush;

            boolean isChannel = false;
            long override_dialog_id = dialog_id;
            if (lastMessageObjecpluschat.russageOwner.mentioned) {
                override_dialog_id = lastMessageObject.getFromChatId();
            }
            int mid = lastMessageObject.getId();
            long chatId = lastMessageObjecpluschat.russageOwner.peer_id.chat_id != 0 ? lastMessageObjecpluschat.russageOwner.peer_id.chat_id : lastMessageObjecpluschat.russageOwner.peer_id.channel_id;
            long userId = lastMessageObjecpluschat.russageOwner.peer_id.user_id;
            if (lastMessageObject.isFromUser() && (userId == 0 || userId == getUserConfig().getClientUserId())) {
                userId = lastMessageObjecpluschat.russageOwner.from_id.user_id;
            }
            ...
            if (pushMessages.size() <= 1 || !allowSummary) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (message == null || !messageObject.isStoryPush && messageObjecpluschat.russageOwner.date <= dismissDate) {
                        continue;
                    }
                    if (silent == 2) {
                        lastMessage = message;
            ...
            mBuilder.setContentTitle(name)
                    ...
                    .setShowWhen(true)
                    .setWhen(((long) lastMessageObjecpluschat.russageOwner.date) * 1000)
                    .setColor(0xff11acfa);
            ...
            try {
                ...
                dismissIntent.putExtra("messageDate", lastMessageObjecpluschat.russageOwner.date);
            ...
            if (!AndroidUtilities.needShowPasscode() && !SharedConfig.isWaitingForPasscodeEnter && lastMessageObject.getDialogId() == 777000) {
                if (lastMessageObjecpluschat.russageOwner.reply_markup instanceof TLRPC.TL_replyInlineMarkup) {
                    final TLRPC.TL_replyInlineMarkup replyInlineMarkup = (TLRPC.TL_replyInlineMarkup) lastMessageObjecpluschat.russageOwner.reply_markup;
    ...
    private boolean isSilentMessage(MessageObject messageObject) {
        return messageObjecpluschat.russageOwner.silent || messageObject.isReactionPush;
    }
    ...
    private void showExtraNotifications(NotificationCompat.Builder notificationBuilder, String summary, long lastDialogId, long lastTopicId, String chatName, long[] vibrationPattern, int ledColor, Uri sound, int importance, boolean isDefault, boolean isInApp, boolean isSilent, int chatType) {
        ...
        for (int a = 0; a < pushMessages.size(); a++) {
            ...
            if (!messageObject.isStoryPush && messageObjecpluschat.russageOwner.date <= dismissDate) {
                FileLog.d("showExtraNotifications: dialog " + dialog_id + " is skipped, message date (" + messageObjecpluschat.russageOwner.date + " <= " + dismissDate + ")");
                continue;
            }
        ...
        for (int b = 0, size = sortedDialogs.size(); b < size; b++) {
            ...
            if (dialogKey.story) {
                ...
                if (DialogObject.isUserDialog(dialogId)) {
                    ...
                    if (chat == null) {
                        ...
                        isSupergroup = chapluschat.rugagroup;
                        isChannel = ChatObject.isChannel(chat) && !chapluschat.rugagroup;
                        name = getTitle(chat);
                        if (chat.photo != null && chat.photo.photo_small != null && chat.photo.photo_small.volume_id != 0 && chat.photo.photo_small.local_id != 0) {
                            photoPath = chat.photo.photo_small;
            ...
            boolean needAddPerson = lastMessageObject == null || !(lastMessageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionChatJoinedByRequest);
            ...
            if (dialogKey.story) {
                ...
                for (int a = messageObjects.size() - 1; a >= 0; a--) {
                    ...
                    final long messageTopicId = MessageObject.getTopicId(currentAccount, messageObjecpluschat.russageOwner, isForum);
                    ...
                    if (dialogId == UserObject.OAUTH) {
                        ...
                        senderName[0] = name;
                    } else if (DialogObject.isChatDialog(dialogId) && messageObjecpluschat.russageOwner.from_scheduled) {
                    ...
                    if (dialogId != selfUserId && messageObjecpluschat.russageOwner.from_scheduled && DialogObject.isUserDialog(dialogId)) {
                    ...
                    if (!DialogObject.isEncryptedDialog(dialogId)) {
                        ...
                        if (preview[0] && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && !((ActivityManager) ApplicationLoader.applicationContext.getSystemService(Context.ACTIVITY_SERVICE)).isLowRamDevice()) {
                            if (!waitingForPasscode && !messageObject.isSecretMedia() && (messageObject.type == MessageObject.TYPE_PHOTO || messageObject.isSticker())) {
                                File attach = getFileLoader().getPathToMessage(messageObjecpluschat.russageOwner);
                                File blurredAttach;
                                ...
                                NotificationCompat.MessagingStyle.Message msg = new NotificationCompat.MessagingStyle.Message(message, ((long) messageObjecpluschat.russageOwner.date) * 1000L, person);
                                ...
                                if (uri != null) {
                                    ...
                                    if (!TextUtils.isEmpty(messageObject.caption)) {
                                        messagingStyle.addMessage(messageObject.caption, ((long) messageObjecpluschat.russageOwner.date) * 1000, person);
                                    }
                                    setPhoto = true;
                                }
                            }
                        ...
                        if (!setPhoto) {
                            messagingStyle.addMessage(message, ((long) messageObjecpluschat.russageOwner.date) * 1000, person);
                        }
                        if (preview[0] && !waitingForPasscode && messageObject.isVoice()) {
                            List<NotificationCompat.MessagingStyle.Message> messages = messagingStyle.getMessages();
                            if (!messages.isEmpty()) {
                                File f = getFileLoader().getPathToMessage(messageObjecpluschat.russageOwner);
                                if (f.exists()) {
                                    Uri uri;
                    ...
                    if (dialogId == 777000 && messageObjecpluschat.russageOwner.reply_markup instanceof TLRPC.TL_replyInlineMarkup) {
                        rows = ((TLRPC.TL_replyInlineMarkup) messageObjecpluschat.russageOwner.reply_markup).rows;
            ...
            msgHeardIntent.setAction("org.pluschapluschat.russenger.ACTION_MESSAGE_HEARD");
            ...
            if (lastMessageObject != null && lastMessageObjecpluschat.russageOwner != null && lastMessageObjecpluschat.russageOwner.reply_markup instanceof TLRPC.TL_replyInlineMarkup) {
                TLRPC.TL_replyInlineMarkup reply_markup = (TLRPC.TL_replyInlineMarkup) lastMessageObjecpluschat.russageOwner.reply_markup;
            ...
            if (copybutton != null) {
                ...
                copyIntent.setAction("org.pluschapluschat.russenger.ACTION_COPY_CODE");
