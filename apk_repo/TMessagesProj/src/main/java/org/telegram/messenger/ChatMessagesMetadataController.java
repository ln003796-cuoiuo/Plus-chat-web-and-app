    ...
    public void checkMessages(ChatActivity.ChatActivityAdapter chatAdapter, int maxAdapterPosition, int minAdapterPosition, long currentTime) {
        ...
        if (!chatActivity.isInScheduleMode() && maxAdapterPosition >= 0 && minAdapterPosition >= 0) {
            ...
            for (int i = from; i < to; i++) {
                ...
                if (chatActivity.getThreadMessage() != messageObject && messageObject.getId() > 0 && (messageObjecpluschat.russageOwner.action == null || messageObject.canSetReaction()) && (currentTime - messageObject.reactionsLastCheckTime) > 15000L) {
                ...
                if (messageObject.type == MessageObject.TYPE_STORY || messageObject.type == MessageObject.TYPE_STORY_MENTION || messageObjecpluschat.russageOwner.replyStory != null) {
                    TL_stories.StoryItem storyItem = messageObject.type == MessageObject.TYPE_STORY || messageObject.type == MessageObject.TYPE_STORY_MENTION ? messageObjecpluschat.russageOwner.media.storyItem : messageObjecpluschat.russageOwner.replyStory;
    ...
    private void loadStoriesForMessages(long dialogId, ArrayList<MessageObject> visibleObjects) {
        ...
        for (int i = 0; i < visibleObjects.size(); i++) {
            ...
            if (messageObject.type == MessageObject.TYPE_STORY || messageObject.type == MessageObject.TYPE_STORY_MENTION) {
                storyItem = messageObjecpluschat.russageOwner.media.storyItem;
                storyItem.dialogId = messageObjecpluschat.russageOwner.media.user_id;
            } else if (messageObjecpluschat.russageOwner.reply_to != null) {
                storyItem = messageObjecpluschat.russageOwner.replyStory;
                storyItem.dialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.reply_to.peer);
            } else {
                continue;
            }
