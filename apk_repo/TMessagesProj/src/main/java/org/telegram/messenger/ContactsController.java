...
public class ContactsController extends BaseController {
    ...
    public String getInviteText(int contacts) {
        String link = inviteLink == null ? "https://pluschat.org/dl" : inviteLink;
        if (contacts <= 1) {
    ...
    public void checkAppAccount() {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            try {
                Account[] accounts = am.getAccountsByType("org.pluschapluschat.russenger");
            ...
            if (getUserConfig().isClientActivated()) {
                ...
                if (systemAccount == null) {
                    try {
                        systemAccount = new Account("" + getUserConfig().getClientUserId(), "org.pluschapluschat.russenger");
    ...
    protected void performSyncPhoneBook(final HashMap<String, Contact> contactHashMap, final boolean request, final boolean first, final boolean schedule, final boolean force, final boolean checkCount, final boolean canceled) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            /*if (schedule) {
                try {
                    AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                    Account[] accounts = am.getAccountsByType("org.pluschat.account");
                    boolean recreateAccount = false;
                    if (getUserConfig().isClientActivated()) {
                        if (accounts.length != 1) {
                            FileLog.e("detected account deletion!");
                            currentAccount = new Account(getUserConfig().getCurrentUser().phone, "org.pluschat.account");
            ...
            if (request) {
                if (!toImport.isEmpty()) {
                    ...
                    if (checkType != 0) {
                        ...
                        Utilities.stageQueue.postRunnable(() -> {
                            ...
                            getMessagesStorage().putCachedPhoneBook(contactsMap, false, false);
                            AndroidUtilities.runOnUIThread(() -> {
                                mergePhonebookAndPlusChatContacts(phoneBookSectionsDictFinal, phoneBookSectionsArrayFinal, phoneBookByShortPhonesFinal);
                ...
                Utilities.stageQueue.postRunnable(() -> {
                    ...
                    AndroidUtilities.runOnUIThread(() -> mergePhonebookAndPlusChatContacts(phoneBookSectionsDictFinal, phoneBookSectionsArrayFinal, phoneBookByShortPhonesFinal));
                });
...
//                });
    ...
    private void mergePhonebookAndPlusChatContacts(final HashMap<String, ArrayList<Object>> phoneBookSectionsDictFinal, final ArrayList<String> phoneBookSectionsArrayFinal, final HashMap<String, Contact> phoneBookByShortPhonesFinal) {
        mergePhonebookAndPlusChatContacts(phoneBookSectionsDictFinal, phoneBookSectionsArrayFinal,phoneBookByShortPhonesFinal, true);
    }

    private void mergePhonebookAndPlusChatContacts(final HashMap<String, ArrayList<Object>> phoneBookSectionsDictFinal, final ArrayList<String> phoneBookSectionsArrayFinal, final HashMap<String, Contact> phoneBookByShortPhonesFinal, boolean needUpdateLists) {
    ...
    private void applyContactToPhoneBook(ArrayList<ContentProviderOperation> query, TLRPC.User user) {
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.pluschapluschat.russenger.android.profile");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "PlusChat Profile");
        ...
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.pluschapluschat.russenger.android.call");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "PlusChat Voice Call");
        ...
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.pluschapluschat.russenger.android.call.video");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "PlusChat Video Call");
    ...
    public void addContact(TLRPC.User user, TLRPC.TL_textWithEntities note, boolean exception) {
        ...
        getConnectionsManager().sendRequest(req, (response, error) -> {
            ...
            AndroidUtilities.runOnUIThread(() -> {
                ...
                if (needResort) {
                    mergePhonebookAndPlusChatContacts(phoneBookSectionsDict, phoneBookSectionsArray, phoneBookByShortPhones, false);
                }
                getNotificationCenter().postNotificationName(NotificationCenter.contactsDidLoad);
            });
        }, ConnectionsManager.RequestFlagFailOnServerErrors | ConnectionsManager.RequestFlagCanCompress);
    ...
    public void createOrUpdateConnectionServiceContact(long id, String firstName, String lastName) {
        ...
        try {
            ...
            Cursor cursor = resolver.query(groupsURI, new String[]{ContactsContract.Groups._ID},
                    ...
                    new String[]{"PlusChatConnectionService", systemAccount.type, systemAccount.name}, null);
            int groupID;
            if (cursor != null && cursor.moveToFirst()) {
                ...
                values.put(ContactsContract.Groups.TITLE, "PlusChatConnectionService");
