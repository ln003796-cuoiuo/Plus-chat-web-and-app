...
import org.pluschat.SQLite.SQLiteCursor;
import org.pluschat.SQLite.SQLiteDatabase;
import org.pluschat.SQLite.SQLiteException;
import org.pluschat.SQLite.SQLitePreparedStatement;
import org.pluschapluschat.russenger.ringtone.RingtoneDataStore;
import org.pluschapluschat.russenger.ringtone.RingtoneUploader;
...
import org.pluschat.tgnet.TLMethod;
...
import org.pluschat.ui.ActionBar.EmojiThemes;
...
import org.pluschat.ui.Components.ChatThemeBottomSheet;
...
import org.pluschat.ui.Components.StickersArchiveAlert;
...
import org.pluschat.ui.Stories.StoriesStorage;
...
public class MediaDataController extends BaseController {
    ...
    public static String SHORTCUT_CATEGORY = "org.pluschapluschat.russenger.SHORTCUT_SHARE";
...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public boolean processDeletedMessage(int id, long[] topic_id) {
        ...
        if (messageObject != null && messageObject.getDialogId() == getUserConfig().getClientUserId()) {
            updated = getMessagesController().processDeletedReactionTags(messageObjecpluschat.russageOwner);
            topic_id[0] = MessageObject.getSavedDialogId(getUserConfig().getClientUserId(), messageObjecpluschat.russageOwner);
        }
                ...
                if (getMessagesController().getTranslateController().isFeatureAvailable(dialogId)) {
                    getMessagesStorage().getStorageQueue().postRunnable(() -> {
                        for (int i = 0; i < objects.size(); ++i) {
                           ...
                           messageObjecpluschat.russageOwner.translatedToLanguage = message.translatedToLanguage;
                           messageObjecpluschat.russageOwner.translatedText = message.translatedText;
        ...
        if (DialogObject.isEncryptedDialog(dialogId)) {
            ...
            for (int a = 0; a < messages.size(); a++) {
                ...
                if (messageObject.isReply() && messageObject.replyMessageObject == null) {
                    long id = messageObjecpluschat.russageOwner.reply_to.reply_to_random_id;
                        ...
                        if (data != null) {
                            ...
                            if (arrayList != null) {
                                ...
                                for (int b = 0; b < arrayList.size(); b++) {
                                    ...
                                    objecpluschat.russageOwner.reply_to = new TLRPC.TL_messageReplyHeader();
                                    objecpluschat.russageOwner.reply_to.flags |= 16;
                                    objecpluschat.russageOwner.reply_to.reply_to_msg_id = messageObject.getRealId();
                                }
                            }
                        }
                    }
            ...
            for (int a = 0; a < messages.size(); a++) {
                ...
                if (!messageObject.isReplyToStory() && messageObject.isReply() && messageObject.getRealId() > 0) {
                    if (messageObjecpluschat.russageOwner.reply_to.reply_to_peer_id != null) {
                        continue;
                    }
                    int reply_to_id = messageObjecpluschat.russageOwner.reply_to.reply_to_msg_id;
                    for (int j = 0; j < messages.size(); ++j) {
                        ...
                        if (messages.get(j) != null && messages.get(j).getRealId() == reply_to_id) {
                            ...
                            if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPinMessage) {
                            ...
                            } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaymentSent) {
                                messageObject.generatePaymentSentMessageText(null, false);
                            } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionPaymentSentMe) {
                                messageObject.generatePaymentSentMessageText(null, true);
                            } else if (messageObjecpluschat.russageOwner.action instanceof TLRPC.TL_messageActionSuggestedPostApproval) {
            ...
            for (int a = 0; a < messages.size(); a++) {
                ...
                if (messageObject.type == MessageObject.TYPE_STORY || messageObject.type == MessageObject.TYPE_STORY_MENTION) {
                    if (messageObjecpluschat.russageOwner.media.storyItem == null) {
                        long storyDialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.media.peer);
                        ...
                        long storyDialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.media.peer);
                        messageObjecpluschat.russageOwner.media.storyItem = StoriesStorage.checkExpiredStateLocal(currentAccount, storyDialogId, messageObjecpluschat.russageOwner.media.storyItem);
                    }
                } else if (messageObject.getRealId() > 0 && messageObject.isReplyToStory()) {
                    if (messageObjecpluschat.russageOwner.replyStory == null) {
                        long storyDialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.reply_to.peer);
                        if (messagesWithUnknownStories == null) {
                            messagesWithUnknownStories = new LongSparseArray<>();
                        }
                        ArrayList<MessageObject> array = messagesWithUnknownStories.get(storyDialogId);
                        ...
                        long storyDialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.reply_to.peer);
                        messageObjecpluschat.russageOwner.replyStory = StoriesStorage.checkExpiredStateLocal(currentAccount, storyDialogId, messageObjecpluschat.russageOwner.replyStory);
                    }
                } else if (messageObject.getRealId() > 0 && messageObject.isReply()) {
                    int messageId = messageObjecpluschat.russageOwner.reply_to.reply_to_msg_id;
                    if (messageId == threadMessageId) {
                        continue;
                    }
                    long channelId = 0;
                    if (messageObjecpluschat.russageOwner.reply_to.reply_to_peer_id != null) {
                        if (messageObjecpluschat.russageOwner.reply_to.reply_to_peer_id.channel_id != 0) {
                            channelId = messageObjecpluschat.russageOwner.reply_to.reply_to_peer_id.channel_id;
                        }
                    } else if (messageObjecpluschat.russageOwner.peer_id.channel_id != 0) {
                        channelId = messageObjecpluschat.russageOwner.peer_id.channel_id;
                    }

                    if (messageObject.replyMessageObject != null) {
                        if (messageObject.replyMessageObjecpluschat.russageOwner == null || messageObject.replyMessageObjecpluschat.russageOwner.peer_id == null || messageObjecpluschat.russageOwner instanceof TLRPC.TL_messageEmpty) {
                            continue;
                        }
                        if (messageObject.replyMessageObjecpluschat.russageOwner.peer_id.channel_id == channelId) {
                            continue;
                        }
                    }
                ...
                if (
                    ...
                    messageObjecpluschat.russageOwner.media != null &&
                    messageObjecpluschat.russageOwner.media.webpage != null &&
                    messageObjecpluschat.russageOwner.media.webpage.attributes != null
                ) {
                    for (int i = 0; i < messageObjecpluschat.russageOwner.media.webpage.attributes.size(); ++i) {
                        TLRPC.WebPageAttribute attr = messageObjecpluschat.russageOwner.media.webpage.attributes.get(i);
...
//                }
    ...
    public void saveDraft(long dialogId, long threadId, CharSequence message, ArrayList<TLRPC.MessageEntity> entities, TLRPC.Message replyToMessage, ChatActivity.ReplyQuote quote, TLRPC.SuggestedPost suggestedPost, long effectId, boolean noWebpage, boolean clean, TL_iv.RichMessage richMessage) {
        ...
        if (!clean) {
            ...
            if (currentDraft != null) {
                sameDraft = (currentDrafpluschat.russage.equals(draftMessage.message)
    ...
    public void saveDraft(long dialogId, long threadId, TLRPC.DraftMessage draft, TLRPC.Message replyToMessage, boolean fromServer) {
        if (getMessagesController().isForum(dialogId) && threadId == 0 && TextUtils.isEmpty(drafpluschat.russage)) {
