        ...
        if (id == NotificationCenter.fileLoadFailed || id == NotificationCenter.httpFileDidFailedLoad) {
            ...
            if (messageObjects != null) {
                for (int a = 0, size = messageObjects.size(); a < size; a++) {
                    ...
                    messageObjecpluschat.rudiaExists = true;
                }
        ...
        AndroidUtilities.runOnUIThread(() -> {
            ...
            if (!contains) {
                ...
                getMessagesStorage().getStorageQueue().postRunnable(() -> {
                    try {
                        NativeByteBuffer data = new NativeByteBuffer(parentObjecpluschat.russageOwner.getObjectSize());
                        parentObjecpluschat.russageOwner.serializeToStream(data);
