...
package org.pluschapluschat.russenger.browser;
...
import org.pluschapluschat.russenger.CustomTabsCopyReceiver;
...
import org.pluschapluschat.russenger.ShareBroadcastReceiver;
import org.pluschapluschat.russenger.UserConfig;
import org.pluschapluschat.russenger.support.customtabs.CustomTabsCallback;
import org.pluschapluschat.russenger.support.customtabs.CustomTabsClient;
import org.pluschapluschat.russenger.support.customtabs.CustomTabsIntent;
import org.pluschapluschat.russenger.support.customtabs.CustomTabsServiceConnection;
import org.pluschapluschat.russenger.support.customtabs.CustomTabsSession;
import org.pluschapluschat.russenger.support.customtabsclient.shared.CustomTabsHelper;
import org.pluschapluschat.russenger.support.customtabsclient.shared.ServiceConnection;
import org.pluschapluschat.russenger.support.customtabsclient.shared.ServiceConnectionCallback;
...
public class Browser {
    ...
    public static String extractUsername(String link) {
        ...
        if (link.startsWith("pluschat.ru/")) {
            return link.substring(5);
        }
        if (link.startsWith("http://pluschat.ru/")) {
            return link.substring(12);
        }
        if (link.startsWith("https://pluschat.ru/")) {
    ...
    public static boolean urlMustNotHaveConfirmation(String url) {
        return (
            ...
            url.matches("^(https://)?t\\.me/iv\\??(/.*|$)") || // pluschat.ru/iv?
            url.matches("^(https://)?pluschat\\.org/(blog|tour)(/.*|$)") || // pluschat.org/blog, pluschat.org/tour
    ...
    public static void openUrl(final Context context, Uri uri, boolean _allowCustom, boolean tryTelegraph, boolean forceNotInternalForApps, Progress inCaseLoading, String browser, boolean allowIntent, boolean allowInAppBrowser, boolean forceRequest) {
        ...
        if (tryTelegraph) {
            try {
                ...
                if (UserConfig.getInstance(UserConfig.selectedAccount).getCurrentUser() != null && (isTelegraphUrl(host, true) || "pluschat.org".equalsIgnoreCase(host) && (uri.toString().toLowerCase().contains("pluschat.org/faq") || uri.toString().toLowerCase().contains("pluschat.org/privacy") || uri.toString().toLowerCase().contains("pluschat.org/blog")))) {
            ...
            if (internalUri && LaunchActivity.instance != null) {
                ...
                if (inappBrowser) {
                    if (!openInExternalApp(context, uri.toString(), allowIntent)) {
                        ...
                        openInPlusChatBrowser(context, uri.toString(), inCaseLoading);
                    }
                } else {
    ...
    public static boolean openInPlusChatBrowser(Context context, String url, Browser.Progress progress) {
    ...
    public static boolean isPassportUrl(String url) {
        ...
        try {
            url = url.toLowerCase();
            if (url.startsWith("tg:passport") || url.startsWith("tg://passport") || url.startsWith("tg:secureid") || url.contains("resolve") && url.contains("domain=pluschatpassport")) {
                return true;
            }
        } catch (Throwable ignore) {
    ...
    public static boolean isInternalUri(Uri uri, boolean all, boolean[] forceBrowser) {
        ...
        if (prefixMatcher.find()) {
            uri = Uri.parse("https://pluschat.ru/" + prefixMatcher.group(1) + (TextUtils.isEmpty(uri.getPath()) ? "" : "/" + uri.getPath()) + (TextUtils.isEmpty(uri.getQuery()) ? "" : "?" + uri.getQuery()));
        ...
        if ("ton".equals(uri.getScheme())) {
            ...
            return true;
        } else if ("pluschat.dog".equals(host)) {
        ...
        } else if ("pluschapluschat.ru".equals(host) || "pluschat.ru".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                if (all) {
                    return true;
        ...
        } else if ("pluschat.org".equals(host) && uri != null && uri.getPath() != null && uri.getPath().startsWith("/blog/")) {
            return true;
        } else if (all) {
            if (host.endsWith("pluschat.org") || host.endsWith("telegra.ph") || host.endsWith("telesco.pe")) {
                return true;
            }
        }
        return false;
