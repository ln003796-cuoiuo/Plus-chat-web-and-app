...
//        }
        ...
        public void addSelfDummyParticipant(boolean notify) {
            ...
            if (ChatObject.canManageCalls(chat) || !ChatObject.isChannel(chat) || chapluschat.rugagroup || selfDummyParticipant.can_self_unmute) {
    ...
    public static boolean canSendAsPeers(TLRPC.Chat chat) {
        return ChatObject.isChannel(chat) && (!chapluschat.rugagroup && chat.signatures && ChatObject.hasAdminRights(chat) && ChatObject.canWriteToChat(chat) || chapluschat.rugagroup && (ChatObject.isPublic(chat) || chat.has_geo || chat.has_link));
    }
    ...
    public static boolean isChannelOrGiga(TLRPC.Chat chat) {
        return isChannel(chat) && (!chapluschat.rugagroup || chat.gigagroup);
    }

    public static boolean isMegagroup(TLRPC.Chat chat) {
        return isChannel(chat) && chapluschat.rugagroup;
    }
    ...
    public static boolean isMegagroup(int currentAccount, long chatId) {
        ...
        return ChatObject.isChannel(chat) && chapluschat.rugagroup;
    }
    ...
    public static boolean canAddBotsToChat(TLRPC.Chat chat) {
        if (isChannel(chat)) {
            if (chapluschat.rugagroup && (chat.admin_rights != null && (chat.admin_rights.post_messages || chat.admin_rights.add_admins) || chat.creator)) {
                return true;
            }
        } else {
    ...
    public static boolean canPinMessages(TLRPC.Chat chat) {
        return (canUserDoAction(chat, ACTION_PIN) || ChatObject.isChannel(chat) && !chapluschat.rugagroup && chat.admin_rights != null && chat.admin_rights.edit_messages);
    }
    ...
    public static boolean isCanWriteToChannel(long chatId, int currentAccount) {
        TLRPC.Chat chat = MessagesController.getInstance(currentAccount).getChat(chatId);
        return ChatObject.canSendMessages(chat) || chapluschat.rugagroup;
    }
