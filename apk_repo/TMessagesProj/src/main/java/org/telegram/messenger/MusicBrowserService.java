...
public class MusicBrowserService extends MediaBrowserService {
    ...
    public void onCreate() {
        ...
        PlusChatMediaSession holder = PlusChatMediaSession.getInstance(this);
    ...
    public BrowserRoot onGetRoot(String clientPackageName, int clientUid, Bundle rootHints) {
        ...
        if (PlusChatMediaSession.getInstance(this).isPasscodeLocked()) {
            return null;
        }
        return new BrowserRoot(MEDIA_ID_ROOT, PlusChatMediaSession.getInstance(this).buildRootHints());
    }
    ...
    public void onLoadChildren(String parentMediaId, Result<List<MediaBrowser.MediaItem>> result) {
        ...
        if (holder.isPasscodeLocked()) {
            Toast.makeText(getApplicationContext(), LocaleController.getString(R.string.EnterYourPlusChatPasscode), Toast.LENGTH_LONG).show();
            stopSelf();
            result.detach();
            return;
        }
