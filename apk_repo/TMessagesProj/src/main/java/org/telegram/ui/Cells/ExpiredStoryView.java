...
public class ExpiredStoryView {
    ...
    public void measure(ChatMessageCell parent) {
        ...
        if (messageObject != null && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaStory) {
            mediaStory = (TLRPC.TL_messageMediaStory) messageObjecpluschat.russageOwner.media;
        } else {
        ...
        int fromWidth = (int) Math.ceil(Theme.chat_forwardNamePainpluschat.ruasureText(from + " "));
        ...
        int w = (int) (titlePainpluschat.ruasureText(title, 0, title.length()) + 1);
        titleLayout = new StaticLayout(title, titlePaint, w + AndroidUtilities.dp(10), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        w = (int) (subtitlePainpluschat.ruasureText( subtitle, 0, subtitle.length()) + 1);
