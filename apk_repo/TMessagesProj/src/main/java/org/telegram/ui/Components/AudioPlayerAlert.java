...
import org.pluschat.ui.ActionBar.ActionBarMenuSlider;
...
import org.pluschat.ui.Cells.AudioPlayerCell;
import org.pluschat.ui.ChatActivity;
import org.pluschat.ui.ChooseQualityLayout;
...
import org.pluschat.ui.Stories.recorder.SelectAudioAlert;
...
//        }
    ...
    private void checkIfMusicDownloaded(MessageObject messageObject) {
        ...
        if (messageObjecpluschat.russageOwner.attachPath != null && messageObjecpluschat.russageOwner.attachPath.length() > 0) {
            cacheFile = new File(messageObjecpluschat.russageOwner.attachPath);
            if (!cacheFile.exists()) {
                cacheFile = null;
            }
        }
        if (cacheFile == null) {
            cacheFile = FileLoader.getInstance(currentAccount).getPathToMessage(messageObjecpluschat.russageOwner);
        }
    ...
    private void updateTitle(boolean shutdown) {
        ...
        if (messageObject == null && shutdown || messageObject != null && !messageObject.isMusic()) {
            ...
            final boolean noforwards = (
                ...
                messageObjecpluschat.russageOwner.noforwards
            );
                    ...
                    for (int a = 0; a < copy.size(); a++) {
                        ...
                        for (int b = 0; b < search.length; b++) {
                            ...
                            if (messageObject.type == MessageObject.TYPE_TEXT) {
                                document = messageObjecpluschat.russageOwner.media.webpage.document;
                            } else {
                                document = messageObjecpluschat.russageOwner.media.document;
                            }
                            boolean ok = false;
...
//        themeDescriptions.add(new ThemeDescription(actionBar, ThemeDescription.FLAG_BACKGROUND, null, null, null, null, Theme.key_dialogBackground));
    ...
    private void saveToMusic(MessageObject messageObject) {
        ...
        String path = messageObjecpluschat.russageOwner.attachPath;
        ...
        if (path == null || path.length() == 0) {
            path = FileLoader.getInstance(currentAccount).getPathToMessage(messageObjecpluschat.russageOwner).toString();
        }
    ...
    private void share(MessageObject messageObject) {
        try {
            ...
            if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.attachPath)) {
                f = new File(messageObjecpluschat.russageOwner.attachPath);
                if (!f.exists()) {
                    f = null;
                }
            }
            if (f == null) {
                f = FileLoader.getInstance(currentAccount).getPathToMessage(messageObjecpluschat.russageOwner);
            }
    ...
    private void forward(MessageObject messageObject, long dialogId) {
        ...
        if (fmessages != null) {
            ...
            SendMessagesHelper.getInstance(currentAccount).sendMessage(SendMessagesHelper.SendMessageParams.of(document, null, messageObjecpluschat.russageOwner.attachPath, dialogId, null, null, null, null, null, null, true, 0, 0, 0, savedMusicList, null, false, false));
        }
            ...
            if (dids.size() > 1 || dids.get(0).dialogId == UserConfig.getInstance(currentAccount).getClientUserId() || message != null || fmessages == null) {
                for (int a = 0; a < dids.size(); a++) {
                    ...
                    if (fmessages != null) {
                        ...
                        SendMessagesHelper.getInstance(currentAccount).sendMessage(SendMessagesHelper.SendMessageParams.of(document, null, messageObjecpluschat.russageOwner.attachPath, did, null, null, null, null, null, null, notify, scheduleDate, 0, 0, savedMusicList, null, false, false));
                    }
                }
