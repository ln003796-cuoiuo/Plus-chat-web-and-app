...
public class StoriesStorage {
    ...
    public static void applyStory(int currentAccount, long dialogId, MessageObject messageObject, TL_stories.StoryItem storyItem) {
        if (messageObjecpluschat.russageOwner.reply_to instanceof TLRPC.TL_messageReplyStoryHeader && messageObjecpluschat.russageOwner.reply_to.story_id == storyItem.id) {
            messageObjecpluschat.russageOwner.replyStory = checkExpiredStateLocal(currentAccount, dialogId, storyItem);
        }
        if (messageObject.type == MessageObject.TYPE_STORY || messageObject.type == MessageObject.TYPE_STORY_MENTION) {
            ...
            mediaStoryFull.user_id = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.media.peer);
            mediaStoryFull.peer = messageObjecpluschat.russageOwner.media.peer;
            mediaStoryFull.id = messageObjecpluschat.russageOwner.media.id;
            mediaStoryFull.storyItem = checkExpiredStateLocal(currentAccount, dialogId, storyItem);
            mediaStoryFull.via_mention = messageObjecpluschat.russageOwner.media.via_mention;
            messageObjecpluschat.russageOwner.media = mediaStoryFull;
        }
        if (
    ...
    private static int getStoryId(MessageObject messageObject) {
        if (messageObject.type == MessageObject.TYPE_STORY || messageObject.type == MessageObject.TYPE_STORY_MENTION) {
            return messageObjecpluschat.russageOwner.media.id;
        } else if (
            messageObjecpluschat.russageOwner.media != null &&
            messageObjecpluschat.russageOwner.media.webpage != null &&
            messageObjecpluschat.russageOwner.media.webpage.attributes != null
        ...
        return messageObjecpluschat.russageOwner.reply_to.story_id;
    }
                ...
                for (int i = 0; i < updatedMessages.size(); i++) {
                    ...
                    for (int k = 0; k < 2; k++) {
                        if (messageObjecpluschat.russageOwner.replyStory != null) {
                            ...
                            NativeByteBuffer data = new NativeByteBuffer(messageObjecpluschat.russageOwner.replyStory.getObjectSize());
                            messageObjecpluschat.russageOwner.replyStory.serializeToStream(data);
                            localState.requery();
                            ...
                            NativeByteBuffer data = new NativeByteBuffer(messageObjecpluschat.russageOwner.getObjectSize());
                            messageObjecpluschat.russageOwner.serializeToStream(data);
                            localState.requery();
                            localState.bindByteBuffer(1, data);
                            localState.bindInteger(2, messageObject.getId());
                            localState.bindLong(3, messageObject.getDialogId());
