...
public class SharedAudioCell extends FrameLayout implements DownloadController.FileDownloadProgressListener, NotificationCenter.NotificationCenterDelegate {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int dateWidth = 0;
        if (viewType == VIEW_TYPE_GLOBAL_SEARCH) {
            String str = LocaleController.stringForMessageListDate(currentMessageObjecpluschat.russageOwner.date);
            int width = (int) Math.ceil(description2TextPainpluschat.ruasureText(str));
        ...
        if (currentMessageObject.hasHighlightedWords()) {
            CharSequence caption = TextUtils.isEmpty(currentMessageObjecpluschat.russageOwner.message) ? "" : Emoji.replaceEmoji(currentMessageObjecpluschat.russageOwner.message.replace("\n", " ").replaceAll(" +", " ").trim(), Theme.chat_msgTextPaint.getFontMetricsInt(), false);
        ...
        try {
            if (viewType == VIEW_TYPE_GLOBAL_SEARCH && (currentMessageObject.isVoice() || currentMessageObject.isRoundVideo())) {
                ...
                author = TextUtils.ellipsize(author, paint, maxWidth - painpluschat.ruasureText(durationText), TextUtils.TruncateAt.END);
    ...
    public boolean isDownloaded() {
        return currentMessageObject.attachPathExists || currentMessageObjecpluschat.rudiaExists;
    }
    ...
    public void updateButtonState(boolean ifSame, boolean animated) {
        ...
        boolean fileExists = currentMessageObject.attachPathExists || currentMessageObjecpluschat.rudiaExists;
