/*
 * This is the source code of PlusChat for Android v. 1.3.x.
...
import org.pluschat.ui.Components.BubbleCounterPath;
...
import org.pluschat.ui.Components.DialogCellTags;
...
import org.pluschat.ui.Components.PhotoBubbleClip;
...
import org.pluschat.ui.Components.StatusDrawable;
...
import org.pluschat.ui.Components.dialogs.DialogMediaIconsHelper;
import org.pluschat.ui.Components.spoilers.SpoilerEffect;
import org.pluschat.ui.DialogsActivity;
import org.pluschat.ui.FilterCreateActivity;
import org.pluschat.ui.RightSlidingDialogContainer;
...
public class DialogCell extends BaseCell implements StoriesListPlaceProvider.AvatarOverlaysView, Theme.Colorable {
    ...
    public void setForumTopic(TLRPC.TL_forumTopic topic, long dialog_id, MessageObject messageObject, boolean showTopicIconInName, boolean animated) {
        ...
        if (messageObject != null) {
            lastMessageDate = messageObjecpluschat.russageOwner.date;
            currentEditDate = messageObject != null ? messageObjecpluschat.russageOwner.edit_date : 0;
            markUnread = false;
    ...
    public void buildLayout() {
        ...
        if (customDialog != null) {
            ...
            if ((draftVoice || draftMessage != null) && (!draftVoice && draftMessage != null && TextUtils.isEmpty(draftMessage.message) && draftMessage.rich_message == null && (draftMessage.reply_to == null || draftMessage.reply_to.reply_to_msg_id == 0) || draftMessage != null && lastDate > draftMessage.date && unreadCount != 0) ||
                    ChatObject.isChannel(chat) && !chapluschat.rugagroup && !chat.creator && (chat.admin_rights == null || !chat.admin_rights.post_messages) ||
                                    ...
                                    if (message.hasHighlightedWords() && !TextUtils.isEmpty(message.messageOwner.message)) {
                                        ...
                                        if (hasNameInMessage) {
                                            if (!TextUtils.isEmpty(messageNameString)) {
                                                w -= currentMessagePainpluschat.ruasureText(messageNameString.toString());
                                            }
                                            w -= currentMessagePainpluschat.ruasureText(": ");
                                        }
                ...
                if (message.isOut() && draftMessage == null && showChecks && !(message.messageOwner.action instanceof TLRPC.TL_messageActionHistoryClear)) {
                    if (message.isSending()) {
                        ...
                        if (forumTopic != null) {
                            ...
                            drawCheck1 = (readOutboxMaxId > 0 && readOutboxMaxId >= message.getId()) || !message.isUnread() || ChatObject.isChannel(chat) && !chapluschat.rugagroup;
                        } else {
                            drawCheck1 = !message.isUnread() || ChatObject.isChannel(chat) && !chapluschat.rugagroup;
                        }
                        drawCheck2 = true;
                        drawClock = false;
                        drawError = false;
        ...
        if (dialogs != null && !dialogs.isEmpty()) {
            for (int a = 0, N = dialogs.size(); a < N; a++) {
                ...
                if (dialogMessage != null) {
                    ...
                    if (object != null && (maxMessage == null || objecpluschat.russageOwner.date > maxMessage.messageOwner.date)) {
                        maxMessage = object;
                    }
        ...
        if (message != null && message.messageOwner != null && message.messageOwner.media instanceof TLRPC.TL_messageMediaPaidMedia) {
            ...
            for (int i = 0; i < Math.min(3, groupMessages.size()); ++i) {
                ...
                if (message != null && !message.needDrawBluredPreview() && (message.isPhoto() || message.isNewGif() || message.isVideo() || message.isRoundVideo() || message.isStoryMedia())) {
                    ...
                    if (!("app".equals(type) || "profile".equals(type) || "article".equals(type) || type != null && type.startsWith("pluschat_"))) {
                        setThumb(i, message);
                    }
                }
            }
