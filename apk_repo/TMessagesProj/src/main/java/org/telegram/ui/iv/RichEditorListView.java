...
import org.pluschat.ui.ActionBar.FloatingToolbar;
...
public class RichEditorListView extends UniversalRecyclerView {
    ...
    private MediaUploadState resolveBlockMedia(TL_iv.PageBlock block) {
        if (block instanceof TL_iv.pageBlockAudio) {
            ...
            final TLRPC.PhotoSize big = org.pluschapluschat.russenger.FileLoader.getClosestPhotoSizeWithSize(photo.sizes, AndroidUtilities.getPhotoSize());
            if (big != null) {
                media.width = big.w;
                media.height = big.h;
            }
    ...
    public class InlineButtonEdit {
        ...
        public void showInputDialog(String title, String hint, String initial, boolean showPaste,
                                    org.pluschat.ui.Components.EditTextCaption.InputDialogCallback callback) {
        ...
        @Override public int getOrderedListMarkerWidth(BlockRow row, android.graphics.Paint paint) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (item.level == level && item.num > 0) {
                    widest = Math.max(widest, measurePainpluschat.ruasureText(item.num + "."));
                }
            }
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            for (int q = 0; q < r.quoteIds.size(); q++) quoteRowCounpluschat.rurge(r.quoteIds.get(q), 1, Integer::sum);
    ...
    private boolean collapseSingleBlockQuotes() {
        ...
        for (int i = 0; i < rows.size(); i++) {
            final BlockRow r = rows.get(i);
            if (r.detailsEnd) continue;
            for (int q = 0; q < r.quoteIds.size(); q++) counpluschat.rurge(r.quoteIds.get(q), 1, Integer::sum);
        }
        boolean changed = false;
        for (int i = 0; i < rows.size(); i++) {
            final BlockRow r = rows.get(i);
