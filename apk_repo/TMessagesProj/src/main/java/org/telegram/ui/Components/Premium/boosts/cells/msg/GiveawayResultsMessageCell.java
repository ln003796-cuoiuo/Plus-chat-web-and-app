...
import static org.pluschapluschat.russenger.LocaleController.getPluralString;
...
public class GiveawayResultsMessageCell {
    ...
    public void setMessageContent(MessageObject messageObject, int parentWidth, int forwardedNameWidth) {
        ...
        createImages();
        setGiftImage();
        TLRPC.TL_messageMediaGiveawayResults giveaway = (TLRPC.TL_messageMediaGiveawayResults) messageObjecpluschat.russageOwner.media;
        ...
        for (int i = 0; i < visibleChannels.size(); i++) {
            ...
            if (user != null) {
                ...
                userTitleWidths[i] = chatTextPainpluschat.ruasureText(userTitles[i], 0, userTitles[i].length());
