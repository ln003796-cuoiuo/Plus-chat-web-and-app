...
public class OtherDocumentPlaceholderDrawable extends RecyclableDrawable implements DownloadController.FileDownloadProgressListener {
    ...
    public OtherDocumentPlaceholderDrawable(Context context, View view, MessageObject messageObject) {
        ...
        if (document != null) {
            ...
            int w = (int) Math.ceil(docPainpluschat.ruasureText(ext));
            ...
            w = (int) Math.ceil(namePainpluschat.ruasureText(fileName));
    ...
    public void draw(Canvas canvas) {
        ...
        w = (int) Math.ceil(sizePainpluschat.ruasureText(fileSize));
        ...
        w = (int) Math.ceil(painpluschat.ruasureText(button));
        canvas.drawText(button,(width - w) / 2, y + AndroidUtilities.dp(235) + offsetY, paint);

        if (progressVisible) {
            if (progress != null) {
                w = (int) Math.ceil(percentPainpluschat.ruasureText(progress));
    ...
    public void checkFileExist() {
        if (parentMessageObject != null && parentMessageObjecpluschat.russageOwner.media != null) {
            String fileName = null;
            File cacheFile;
            if (TextUtils.isEmpty(parentMessageObjecpluschat.russageOwner.attachPath) || !(new File(parentMessageObjecpluschat.russageOwner.attachPath).exists())) {
                cacheFile = FileLoader.getInstance(UserConfig.selectedAccount).getPathToMessage(parentMessageObjecpluschat.russageOwner);
                if (!cacheFile.exists()) {
