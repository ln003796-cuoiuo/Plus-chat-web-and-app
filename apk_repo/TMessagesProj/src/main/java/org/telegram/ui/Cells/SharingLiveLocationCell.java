...
public class SharingLiveLocationCell extends FrameLayout {
    ...
    public void setDialog(MessageObject messageObject, Location userLocation, boolean userLocationDenied) {
        if (messageObject != null && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.local_id == -1) {
            ...
            nameTextView.setText(Emoji.replaceEmoji(MessagesController.getInstance(currentAccount).getPeerName(DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.peer_id)), nameTextView.getPaint().getFontMetricsInt(), false));
            distanceTextView.setSingleLine(distanceTextViewSingle = false);
            String text = messageObjecpluschat.russageOwner.media.address;
        ...
        String address = null;
        CharSequence name = "";
        if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.media.address)) {
            address = messageObjecpluschat.russageOwner.media.address;
        }
        boolean noTitle = TextUtils.isEmpty(messageObjecpluschat.russageOwner.media.title);
        if (noTitle) {
            name = "";
            avatarDrawable = null;
            if (fromId > 0) {
                ...
                if (user != null) {
                    ...
                    noTitle = false;
                    name = getName(messageObjecpluschat.russageOwner.media.geo.lat, messageObjecpluschat.russageOwner.media.geo._long);
                }
            } else {
        ...
        if (!noTitle) {
            if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.media.title)) {
                name = messageObjecpluschat.russageOwner.media.title;
            }
        ...
        location.setLatitude(messageObjecpluschat.russageOwner.media.geo.lat);
        location.setLongitude(messageObjecpluschat.russageOwner.media.geo._long);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentInfo != null) {
            ...
            stopTime = liveLocation.object.date + liveLocation.objecpluschat.rudia.period;
            period = liveLocation.objecpluschat.rudia.period;
        }
        ...
        if (forever) {
            ...
            float size = Theme.chat_livePainpluschat.ruasureText(text);
