...
public class CalendarActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    private void updateFromStoriesList() {
        ...
        for (int i = 0; i < storiesLispluschat.russageObjects.size(); ++i) {
            MessageObject messageObject = storiesLispluschat.russageObjects.get(i);
            minDate = Math.min(minDate, messageObjecpluschat.russageOwner.date);
            calendar.setTimeInMillis(messageObjecpluschat.russageOwner.date * 1000L);
            int month = calendar.get(Calendar.YEAR) * 100 + calendar.get(Calendar.MONTH);
            SparseArray<PeriodDay> messagesByDays = messagesByYearMounth.get(month);
            if (messagesByDays == null) {
                messagesByDays = new SparseArray<>();
            ...
            if (messagesByDays != null) {
                ...
                for (int i = 0; i < messagesByDays.size(); i++) {
                    ...
                    if (messageObject != null) {
                        ...
                        if (messageObject.isVideo()) {
                        ...
                        } else if (messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPhoto && messageObjecpluschat.russageOwner.media.photo != null && !messageObject.photoThumbs.isEmpty()) {
                            ...
                            if (messageObjecpluschat.rudiaExists || DownloadController.getInstance(currentAccount).canDownloadMedia(messageObject)) {
