...
import org.pluschat.ui.Components.PlayPauseDrawable;
import org.pluschat.ui.Components.RLottieDrawable;
import org.pluschat.ui.Components.Scroller;
import org.pluschat.ui.Components.TimerParticles;
import org.pluschat.ui.Components.TranslateAlert2;
import org.pluschat.ui.Components.VideoPlayer;
import org.pluschat.ui.Components.VideoPlayerSeekBar;
...
public class SecretMediaViewer implements NotificationCenter.NotificationCenterDelegate, GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    private class SecretDeleteTimer extends FrameLayout {
        ...
        @Override
        protected void onDraw(Canvas canvas) {
            if (currentMessageObject == null || currentMessageObjecpluschat.russageOwner.destroyTime == 0 && currentMessageObjecpluschat.russageOwner.ttl != 0x7FFFFFFF) {
                return;
            }
            ...
            for (int i = 0; i < mids.size(); i++) {
                ...
                for (int a = 0; a < arr.size(); a++) {
                    ...
                    if (currentMessageObject.getId() == mid) {
                        currentMessageObjecpluschat.russageOwner.destroyTime = key;
...
//        dontChangeCaptionPosition = animated && isCaptionEmpty;
        ...
        if (!isCaptionEmpty) {
            ...
            CharSequence str;
            if (messageObject != null /*&& captionTranslated*/ && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.translatedText != null && TextUtils.equals(messageObjecpluschat.russageOwner.translatedToLanguage, TranslateAlert2.getToLanguage())) {
                str = caption;
            } else if (messageObject != null && !messageObjecpluschat.russageOwner.entities.isEmpty()) {
...
//                            if (username != null) {
//                                url1 = "https://pluschat.ru/" + username + "/" + messageId + "?t=" + finalTimestamp;
//                            }
//                        } else {
...
//        bottomSheet.scrollNavBar = true;
    ...
    public void openMedia(MessageObject messageObject, PhotoViewer.PhotoViewerProvider provider, Runnable onOpen, Runnable onClose) {
        ...
        //messageObjecpluschat.russageOwner.destroyTime = (int) (System.currentTimeMillis() / 1000 + ConnectionsManager.getInstance().getTimeDifference()) + 4;

        ignoreDelete = messageObjecpluschat.russageOwner.ttl == 0x7FFFFFFF;
        ...
        currentDialogId = MessageObject.getPeerId(messageObjecpluschat.russageOwner.peer_id);
        ...
        if (document != null) {
            ...
            if (MessageObject.isGifDocument(document)) {
                ...
                if (messageObjecpluschat.russageOwner.attachPath != null && messageObject.attachPathExists) {
                    location = ImageLocation.getForPath(messageObjecpluschat.russageOwner.attachPath);
                } else {
                ...
                if (f.exists()) {
                    ...
                    File file = FileLoader.getInstance(currentAccount).getPathToMessage(messageObjecpluschat.russageOwner);
        ...
        photoAnimationEndRunnable = () -> {
            ...
            containerView.invalidate();
            secretDeleteTimer.setDestroyTime(messageObjecpluschat.russageOwner.destroyTimeMillis, messageObjecpluschat.russageOwner.ttl, false);
    ...
    public boolean closePhoto(boolean animated, boolean byDelete) {
        ...
        if (currentProvider == null || currentMessageObjecpluschat.russageOwner.media.photo instanceof TLRPC.TL_photoEmpty || currentMessageObjecpluschat.russageOwner.media.document instanceof TLRPC.TL_documentEmpty) {
            object = null;
        } else {
