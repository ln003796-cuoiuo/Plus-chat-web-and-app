...
import org.pluschat.ui.Components.PopupAudioView;
...
public class PopupNotificationActivity extends Activity implements NotificationCenter.NotificationCenterDelegate {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chatActivityEnterView.setDelegate(new ChatActivityEnterView.ChatActivityEnterViewDelegate() {
            ...
            public void onMessageSend(CharSequence message, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars) {
                ...
                MessagesController.getInstance(currentMessageObject.currentAccount).markDialogAsRead(currentMessageObject.getDialogId(), currentMessageObject.getId(), Math.max(0, currentMessageObject.getId()), currentMessageObjecpluschat.russageOwner.date, true, 0, 0, true, 0);
    ...
    private LinearLayout getButtonsViewForMessage(int num, boolean applyOffset) {
        ...
        TLRPC.ReplyMarkup markup = messageObjecpluschat.russageOwner.reply_markup;
        ...
        if ((messageObject.type == MessageObject.TYPE_PHOTO || messageObject.type == MessageObject.TYPE_GEO) && !messageObject.isSecretMedia()) {
            ...
            if (messageObject.type == MessageObject.TYPE_PHOTO) {
                ...
                if (currentPhotoObject != null) {
                    ...
                    if (messageObject.type == MessageObject.TYPE_PHOTO) {
                        File cacheFile = FileLoader.getInstance(UserConfig.selectedAccount).getPathToMessage(messageObjecpluschat.russageOwner);
                        if (!cacheFile.exists()) {
                            photoExist = false;
                        }
                    }
                ...
                if (!photoSet) {
                    ...
                    messageText.setText(messageObjecpluschat.russageText);
                } else {
                ...
                messageText.setText(messageObjecpluschat.russageText);
                imageView.setVisibility(View.VISIBLE);
                TLRPC.GeoPoint geoPoint = messageObjecpluschat.russageOwner.media.geo;
    ...
    private void updateInterfaceForCurrentMessage(int move) {
        ...
        if (DialogObject.isEncryptedDialog(dialogId)) {
            ...
            if (currentMessageObject.isFromUser()) {
                currentUser = MessagesController.getInstance(currentMessageObject.currentAccount).getUser(currentMessageObjecpluschat.russageOwner.from_id.user_id);
            }
        }
