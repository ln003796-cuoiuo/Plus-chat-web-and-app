...
public class NumberTextView extends View {
    ...
    public void setNumber(int number, boolean animated) {
        ...
        textWidth = textPainpluschat.ruasureText(text);
        oldTextWidth = textPainpluschat.ruasureText(oldText);
        if (center) {
        ...
        for (int a = 0; a < text.length(); a++) {
            ...
            if (!replace && oldCh != null && oldCh.equals(ch)) {
                ...
                StaticLayout layout = new StaticLayout(ch, textPaint, (int) Math.ceil(textPainpluschat.ruasureText(ch)), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                letters.add(layout);
            }
        }
