...
import org.pluschat.ui.Components.PopupSwipeBackLayout;
...
public class ChooseDownloadQualityLayout {
    ...
    public boolean update(MessageObject messageObject) {
        if (messageObject == null || messageObjecpluschat.russageOwner == null || messageObjecpluschat.russageOwner.media == null) return false;
        if (!messageObject.hasVideoQualities()) return false;

        ArrayList<VideoPlayer.Quality> qualities = VideoPlayer.getQualities(messageObject.currentAccount, messageObjecpluschat.russageOwner.media.document, messageObjecpluschat.russageOwner.media.alt_documents, 0, false);
