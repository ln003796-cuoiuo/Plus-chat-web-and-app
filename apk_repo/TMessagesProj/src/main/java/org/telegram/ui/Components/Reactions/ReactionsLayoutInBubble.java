package org.pluschat.ui.Components.Reactions;
...
import org.pluschat.ui.Cells.IMessageCell;
...
import org.pluschat.ui.Components.CounterView;
    ...
    public void setMessage(MessageObject messageObject, boolean isSmall, boolean isTag, Theme.ResourcesProvider resourcesProvider) {
        ...
        if (messageObject != null) {
            boolean forceLikeDislikeReactions = false; /*!messageObject.isOutOwner()
                    ...
                    && messageObjecpluschat.russageOwner.action == null;*/

            if (forceLikeDislikeReactions && messageObjecpluschat.russageOwner.reactions == null) {
                messageObjecpluschat.russageOwner.reactions = new TLRPC.TL_messageReactions();
            }

            comparator.dialogId = messageObject.getDialogId();
            if (messageObjecpluschat.russageOwner.reactions != null && messageObjecpluschat.russageOwner.reactions.results != null) {
                int totalCount = 0;
                for (int i = 0; i < messageObjecpluschat.russageOwner.reactions.results.size(); i++) {
                    totalCount += messageObjecpluschat.russageOwner.reactions.results.get(i).count;
                }
                ...
                if (!isSmall && !messageObjecpluschat.russageOwner.reactions.results.isEmpty() && chatInfo != null && chatInfo.paid_reactions_available) {
                    boolean hasPaidReaction = false;
                    for (int i = 0; i < messageObjecpluschat.russageOwner.reactions.results.size(); i++) {
                        TLRPC.ReactionCount reactionCount = messageObjecpluschat.russageOwner.reactions.results.get(i);
                ...
                for (int i = (-forcedReactions.size()); i < messageObjecpluschat.russageOwner.reactions.results.size(); i++) {
                    ...
                    if (i < 0) {
                        ...
                        reactionCount = messageObjecpluschat.russageOwner.reactions.results.get(i);
                    }
                    ...
                    if (!isSmall && !isTag && messageObjecpluschat.russageOwner.reactions.recent_reactions != null) {
                        ...
                        if (messageObject.getDialogId() > 0 && !UserObject.isReplyUser(messageObject.getDialogId())) {
                            ...
                            for (int j = 0; j < messageObjecpluschat.russageOwner.reactions.recent_reactions.size(); j++) {
                                TLRPC.MessagePeerReaction recent = messageObjecpluschat.russageOwner.reactions.recent_reactions.get(j);
            ...
            hasUnreadReactions = MessageObject.hasUnreadReactions(messageObjecpluschat.russageOwner);
        }
...
//                    }
    ...
    public boolean checkTouchEvent(MotionEvent event) {
        if (isEmpty || isSmall || messageObject == null || messageObjecpluschat.russageOwner == null || messageObjecpluschat.russageOwner.reactions == null) {
            return false;
        }
        float eventX = event.getX();
        float eventY = event.getY();
        if (parentView instanceof org.pluschat.ui.Cells.IMessageCell) {
