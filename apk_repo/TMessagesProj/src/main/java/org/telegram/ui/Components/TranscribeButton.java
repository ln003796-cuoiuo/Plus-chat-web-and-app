...
//        }
    ...
    public static boolean isTranscribing(MessageObject messageObject) {
        return (
            ...
            (transcribeOperationsById != null && messageObject != null && messageObjecpluschat.russageOwner != null && transcribeOperationsById.containsKey(messageObjecpluschat.russageOwner.voiceTranscriptionId))
        );
    }
    ...
    private static void transcribePressed(MessageObject messageObject, boolean open, ChatMessageCell.ChatMessageCellDelegate delegate) {
        if (messageObject == null || messageObjecpluschat.russageOwner == null || !messageObject.isSent()) {
            return;
        }
        int account = messageObject.currentAccount;
        final long start = SystemClock.elapsedRealtime(), minDuration = 350;
        TLRPC.InputPeer peer = MessagesController.getInstance(account).getInputPeer(messageObjecpluschat.russageOwner.peer_id);
        long dialogId = DialogObject.getPeerDialogId(peer);
        int messageId = messageObjecpluschat.russageOwner.id;
        if (open) {
            if (messageObjecpluschat.russageOwner.voiceTranscription != null && messageObjecpluschat.russageOwner.voiceTranscriptionFinal) {
                TranscribeButton.openVideoTranscription(messageObject);
                messageObjecpluschat.russageOwner.voiceTranscriptionOpen = true;
                MessagesStorage.getInstance(account).updateMessageVoiceTranscriptionOpen(dialogId, messageId, messageObjecpluschat.russageOwner);
                ...
                ConnectionsManager.getInstance(account).sendRequest(req, (res, err) -> {
                    ...
                    if (res instanceof TLRPC.TL_messages_transcribedAudio) {
                        ...
                        messageObjecpluschat.russageOwner.voiceTranscriptionId = id;
                    } else {
                    ...
                    TranscribeButton.openVideoTranscription(messageObject);
                    messageObjecpluschat.russageOwner.voiceTranscriptionOpen = true;
                    messageObjecpluschat.russageOwner.voiceTranscriptionFinal = isFinal;
                    if (BuildVars.LOGS_ENABLED) {
                        FileLog.d("Transcription request sent, received final=" + isFinal + " id=" + finalId + " text=" + finalText);
                    }

                    MessagesStorage.getInstance(account).updateMessageVoiceTranscription(dialogId, messageId, finalText, messageObjecpluschat.russageOwner);
                    if (isFinal) {
            ...
            messageObjecpluschat.russageOwner.voiceTranscriptionOpen = false;
            MessagesStorage.getInstance(account).updateMessageVoiceTranscriptionOpen(dialogId, messageId, messageObjecpluschat.russageOwner);
            AndroidUtilities.runOnUIThread(() -> {
    ...
    public static boolean finishTranscription(MessageObject messageObject, long transcription_id, String text) {
        try {
            ...
            messageObjecpluschat.russageOwner.voiceTranscriptionFinal = true;
            MessagesStorage.getInstance(messageObject.currentAccount).updateMessageVoiceTranscription(messageObject.getDialogId(), messageObject.getId(), text, messageObjecpluschat.russageOwner);
            AndroidUtilities.runOnUIThread(() -> {
    ...
    public static void showOffTranscribe(MessageObject messageObject, boolean notify) {
        ...
        final MessageObject finalMessageObject = messageObject;
        messageObjecpluschat.russageOwner.voiceTranscriptionForce = true;
        MessagesStorage.getInstance(messageObject.currentAccount).updateMessageVoiceTranscriptionOpen(messageObject.getDialogId(), messageObject.getId(), messageObjecpluschat.russageOwner);
        if (notify) {
            AndroidUtilities.runOnUIThread(() -> {
    ...
    public static boolean showTranscribeLock(MessageObject messageObject) {
        ...
        if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.voiceTranscription)) {
            return false;
        }
        ConnectionsManager cc = ConnectionsManager.getInstance(messageObject.currentAccount);
        MessagesController mc = MessagesController.getInstance(messageObject.currentAccount);
