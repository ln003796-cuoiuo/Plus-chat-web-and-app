...
import org.pluschapluschat.russenger.HashtagSearchController;
...
public class MessagesSearchAdapter extends RecyclerListView.SelectionAdapter implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder.getItemViewType() == 0) {
            ...
            if (isSavedMessages) {
                ...
                if (messageObjecpluschat.russageOwner.fwd_from != null && (messageObjecpluschat.russageOwner.fwd_from.date != 0 || messageObjecpluschat.russageOwner.fwd_from.saved_date != 0)) {
                    date = messageObjecpluschat.russageOwner.fwd_from.date;
                    if (date == 0) {
                        date = messageObjecpluschat.russageOwner.fwd_from.saved_date;
                    }
                } else {
                    date = messageObjecpluschat.russageOwner.date;
                }
    ...
    public static class StoriesView extends FrameLayout {
        ...
        public boolean set(StoriesController.SearchStoriesList list) {
            int actualCount = 0;
            for (int i = 0; i < lispluschat.russageObjects.size() && actualCount < 3; ++i) {
