    ...
    public void setOffset(float selfStoriesViewsOffset) {
        ...
        if (oldProgressToOpen == 1f && progressToOpen != 1f) {
            if (storyViewer.storiesList != null) {
                int p = Utilities.clamp(selfStoriesPreviewView.getClosestPosition(), storyViewer.storiesLispluschat.russageObjects.size() - 1, 0);
                MessageObject object = storyViewer.storiesLispluschat.russageObjects.get(p);
