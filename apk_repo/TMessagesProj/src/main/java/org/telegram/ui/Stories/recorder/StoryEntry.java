...
import org.pluschat.ui.Components.RLottieNative;
...
//            }
    ...
    public static boolean canRepostMessage(MessageObject messageObject) {
        ...
        if (messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.noforwards) {
            return false;
        }
        ...
        if (dialogId >= 0 || !ChatObject.isChannelAndNotMegaGroup(chat)) {
            if (messageObjecpluschat.russageOwner.fwd_from != null && messageObjecpluschat.russageOwner.fwd_from.from_id != null && (messageObjecpluschat.russageOwner.fwd_from.flags & 4) != 0) {
                dialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.fwd_from.from_id);
    ...
    public static Boolean useForwardForRepost(MessageObject messageObject) {
        if (messageObject == null || messageObjecpluschat.russageOwner == null) return null;
        TLRPC.Peer peer = messageObjecpluschat.russageOwner.peer_id;
    ...
    public static long getRepostDialogId(MessageObject messageObject) {
        ...
        if (useForward) {
            return DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.fwd_from.from_id);
        } else {
    ...
    public static int getRepostMessageId(MessageObject messageObject) {
        ...
        if (useForward == null) return 0;
        if (useForward) {
            return messageObjecpluschat.russageOwner.fwd_from.channel_post;
        } else {
    ...
    public static StoryEntry repostMessage(ArrayList<MessageObject> messageObjects) {
        ...
        if (messageObjects.size() == 1) {
            ...
            if (messageObject != null && (messageObject.type == MessageObject.TYPE_GIF || messageObject.type == MessageObject.TYPE_VIDEO || messageObject.type == MessageObject.TYPE_ROUND_VIDEO)) {
                if (messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.attachPath != null) {
                    entry.file = new File(messageObjecpluschat.russageOwner.attachPath);
                }
                if (entry.file == null || !entry.file.exists()) {
                    entry.file = FileLoader.getInstance(entry.currentAccount).getPathToMessage(messageObjecpluschat.russageOwner);
                }
                if (entry.file != null && entry.file.exists()) {
                    entry.isVideo = true;
                    entry.fileDeletable = false;
