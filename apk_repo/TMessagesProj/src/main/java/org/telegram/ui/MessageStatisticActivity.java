...
public class MessageStatisticActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public MessageStatisticActivity(MessageObject message) {
        ...
        if (messageObjecpluschat.russageOwner.fwd_from == null) {
            ...
            messageId = messageObjecpluschat.russageOwner.fwd_msg_id;
        }
    ...
    public View createView(Context context) {
        ...
        if (!messageObject.isStory()) {
            if (!messageObject.needDrawBluredPreview() && (messageObject.isPhoto() || messageObject.isNewGif() || messageObject.isVideo())) {
                String type = messageObject.isWebpage() ? messageObjecpluschat.russageOwner.media.webpage.type : null;
                if (!("app".equals(type) || "profile".equals(type) || "article".equals(type) || type != null && type.startsWith("pluschat_"))) {
                    ...
                    if (smallThumb != null) {
                        ...
                        if (messageObjecpluschat.rudiaExists || DownloadController.getInstance(currentAccount).canDownloadMedia(messageObject) || FileLoader.getInstance(currentAccount).isLoadingFile(fileName)) {
                            int size;
            ...
            } else if (!TextUtils.isEmpty(messageObjecpluschat.russageOwner.message)) {
                message = messageObjecpluschat.russageText;
    ...
    public class OverviewCell extends LinearLayout {
        ...
        public void setData() {
            ...
            if (recentPostInfo != null) {
                ...
                views = messageObject.isStory() ? messageObject.storyItem.views.views_count : messageObjecpluschat.russageOwner.views;
                forwards = messageObject.isStory() ? messageObject.storyItem.views.forwards_count : messageObjecpluschat.russageOwner.forwards;
                reactions = 0;
                if (messageObject.isStory()) {
                    reactions = messageObject.storyItem.views.reactions_count;
                } else {
                    if (messageObjecpluschat.russageOwner.reactions != null) {
                        for (int i = 0; i < messageObjecpluschat.russageOwner.reactions.results.size(); i++) {
                            reactions += messageObjecpluschat.russageOwner.reactions.results.get(i).count;
                        }
                    }
                }
            }
