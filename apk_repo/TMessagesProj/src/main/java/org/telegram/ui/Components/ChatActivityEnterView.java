...
import static org.pluschat.ui.LaunchActivity.getLastFragment;
...
import org.pluschat.ui.recyclerview.ChatListItemAnimator;
import androidx.recyclerview.widget.LinearLayoutManager;

import org.pluschapluschat.russenger.AccountInstance;
import org.pluschapluschat.russenger.AndroidUtilities;
import org.pluschapluschat.russenger.AnimationNotificationsLocker;
import org.pluschapluschat.russenger.ApplicationLoader;
import org.pluschapluschat.russenger.BirthdayController;
import org.pluschapluschat.russenger.BotForumHelper;
import org.pluschapluschat.russenger.BotWebViewVibrationEffect;
...
import org.pluschapluschat.russenger.RichMessageLayout;
...
import org.pluschapluschat.russenger.SharedPrefsHelper;
...
import org.pluschapluschat.russenger.VideoEditedInfo;
import org.pluschapluschat.russenger.browser.Browser;
import org.pluschapluschat.russenger.camera.CameraController;
...
import org.pluschat.ui.ActionBar.AdjustPanLayoutHelper;
...
import org.pluschat.ui.BasePermissionsActivity;
import org.pluschat.ui.Business.BusinessLinksController;
...
import org.pluschat.ui.Components.Premium.boosts.BoostRepository;
...
import org.pluschat.ui.Components.chat.SendButtonBlockedByTypingView;
import org.pluschat.ui.Components.chat.layouts.ChatActivitySideControlsButtonsLayout;
import org.pluschat.ui.Components.inset.WindowInsetsInAppController;
import org.pluschat.ui.ContentPreviewViewer;
import org.pluschat.ui.DialogsActivity;
import org.pluschat.ui.Gifts.GiftSheet;
import org.pluschat.ui.GroupStickersActivity;
import org.pluschat.ui.LaunchActivity;
import org.pluschat.ui.LinkManager;
import org.pluschat.ui.MessageSendPreview;
import org.pluschat.ui.MultiContactsSelectorBottomSheet;
import org.pluschat.ui.PhotoViewer;
import org.pluschat.ui.PremiumPreviewFragment;
...
import org.pluschat.ui.StickersActivity;
import org.pluschat.ui.Stories.HighlightMessageSheet;
import org.pluschat.ui.Stories.recorder.CaptionContainerView;
import org.pluschat.ui.Stories.recorder.HintView2;
import org.pluschat.ui.bots.BotCommandsMenuContainer;
import org.pluschat.ui.bots.BotCommandsMenuView;
import org.pluschat.ui.bots.BotKeyboardView;
import org.pluschat.ui.bots.BotWebViewAttachedSheet;
import org.pluschat.ui.bots.BotWebViewSheet;
import org.pluschat.ui.bots.ChatActivityBotWebViewButton;
import org.pluschat.ui.bots.WebViewRequestProps;
import org.pluschat.ui.iv.BlockRow;
import org.pluschat.ui.iv.RichEditor;
import org.pluschat.ui.iv.RichHtml;
import org.pluschat.ui.iv.RichMessageConvert;
...
//                    list.add(2);
    ...
    public ChatActivityEnterView(Activity context, SizeNotifierFrameLayout parent, ChatActivity fragment, final boolean isChat, Theme.ResourcesProvider resourcesProvider) {
        ...
        sendOutlineView.setColorFilter(getThemedColor(Theme.key_pluschat_color), PorterDuff.Mode.SRC_IN);
...
//        audioVideoSendButton.setAccessibilityDelegate(mediaMessageButtonsDelegate);
    ...
    public int getMessagesCount() {
        int count = 0;
        if (parentFragment != null && parentFragmenpluschat.russagePreviewParams != null) {
            count += parentFragmenpluschat.russagePreviewParams.getForwardedMessagesCount();
        }
...
//            }
    ...
    private boolean onSendLongClick(View view) {
        ...
        if (isStories || (messageEditText == null || TextUtils.isEmpty(messageEditText.getText())) && parentFragment != null && parentFragmenpluschat.russagePreviewParams != null && parentFragmenpluschat.russagePreviewParams.forwardMessages != null && parentFragmenpluschat.russagePreviewParams.forwardMessages.messages != null && !parentFragmenpluschat.russagePreviewParams.forwardMessages.messages.isEmpty()) {
            ...
            sendPopupLayoupluschat.ruasure(MeasureSpec.makeMeasureSpec(dp(1000), MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(dp(1000), MeasureSpec.AT_MOST));
        ...
        if (richDraftMessage != null) {
            ...
            if (messageWebPage != null) {
                ...
                if (parentFragment != null && parentFragmenpluschat.russagePreviewParams != null && parentFragmenpluschat.russagePreviewParams.hasMedia) {
                    media.force_small_media = parentFragmenpluschat.russagePreviewParams.webpageSmall;
                    media.force_large_media = !parentFragmenpluschat.russagePreviewParams.webpageSmall;
                    message.invert_media = parentFragmenpluschat.russagePreviewParams.webpageTop;
                }
                message.media = media;
            }
...
//        if (parentFragment != null && parentFragmenpluschat.russagePreviewParams != null && parentFragmenpluschat.russagePreviewParams.forwardMessages != null && parentFragmenpluschat.russagePreviewParams.forwardMessages.messages != null && !parentFragmenpluschat.russagePreviewParams.forwardMessages.messages.isEmpty()) {
//            for (MessageObject msg : parentFragmenpluschat.russagePreviewParams.forwardMessages.messages) {
        ...
        if (parentFragment != null && delegate != null && ChatObject.isMonoForum(parentFragment.getCurrentChat())) {
            ...
            options.add(R.drawable.input_suggest_paid_24, getString(R.string.PostSuggestionsSendWithOffer), () -> {
                new StarsSuggestionSheet(getContext(), currentAccount, parentFragment.getDialogId(),
                        parentFragmenpluschat.russageSuggestionParams != null ? parentFragmenpluschat.russageSuggestionParams: MessageSuggestionParams.empty(),
                        parentFragment, resourcesProvider, StarsSuggestionSheet.MODE_INPUT, (params) -> {

                    parentFragmenpluschat.russageSuggestionParams = params;
...
//        }
    ...
    public void updateFieldHint(boolean animated) {
        ...
        } else if (replyingMessageObject != null && replyingMessageObjecpluschat.russageOwner.reply_markup != null && !TextUtils.isEmpty(replyingMessageObjecpluschat.russageOwner.reply_markup.placeholder)) {
            messageEditText.setHintText(replyingMessageObjecpluschat.russageOwner.reply_markup.placeholder, animated);
        ...
        } else if (botKeyboardViewVisible && botButtonsMessageObject != null && botButtonsMessageObjecpluschat.russageOwner.reply_markup != null && !TextUtils.isEmpty(botButtonsMessageObjecpluschat.russageOwner.reply_markup.placeholder)) {
            messageEditText.setHintText(botButtonsMessageObjecpluschat.russageOwner.reply_markup.placeholder, animated);
...
//        }
    ...
    protected boolean sendMessageInternal(boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, boolean allowConfirm) {
        ...
        final Runnable send = () -> {
            ...
            if (parentFragment != null) {
                ...
                if (user != null || ChatObject.isChannel(chat) && chapluschat.rugagroup || !ChatObject.isChannel(chat)) {
    ...
    public void doneEditingMessage() {
        ...
        if (editingMessageObject.needResendWhenEdit() && !ChatObject.canManageMonoForum(currentAccount, editingMessageObject.getDialogId())) {
            final MessageSuggestionParams params = parentFragment != null && parentFragmenpluschat.russageSuggestionParams != null ?
                parentFragmenpluschat.russageSuggestionParams : MessageSuggestionParams.of(editingMessageObjecpluschat.russageOwner.suggested_post);
        ...
        CharSequence[] message = new CharSequence[]{text};
        if (TextUtils.isEmpty(message[0]) && (editingMessageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaWebPage || editingMessageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaEmpty || editingMessageObjecpluschat.russageOwner.media == null)) {
        ...
        if (!TextUtils.equals(message[0], editingMessageObjecpluschat.russageText) || entities != null && !entities.isEmpty() || !editingMessageObjecpluschat.russageOwner.entities.isEmpty() || editingMessageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaWebPage) {
            ...
            if (parentFragment != null && parentFragment.getCurrentChat() != null && (editingMessageObject.type == MessageObject.TYPE_TEXT || editingMessageObject.type == MessageObject.TYPE_EMOJIS) && !ChatObject.canSendEmbed(parentFragment.getCurrentChat())) {
                ...
                editingMessageObjecpluschat.russageOwner.flags &=~ 512;
                editingMessageObjecpluschat.russageOwner.media = null;
            } else if (parentFragment != null && parentFragmenpluschat.russagePreviewParams != null) {
                if (parentFragment.foundWebPage instanceof TLRPC.TL_webPagePending) {
                    ...
                    if (editingMessageObject.type == MessageObject.TYPE_TEXT || editingMessageObject.type == MessageObject.TYPE_EMOJIS) {
                        editingMessageObjecpluschat.russageOwner.media = new TLRPC.TL_messageMediaEmpty();
                        editingMessageObjecpluschat.russageOwner.flags |= 512;
                    }
                } else if (parentFragmenpluschat.russagePreviewParams.webpage != null) {
                    editingMessageObject.editingMessageSearchWebPage = false;
                    editingMessageObjecpluschat.russageOwner.flags |= 512;
                    editingMessageObjecpluschat.russageOwner.media = new TLRPC.TL_messageMediaWebPage();
                    editingMessageObjecpluschat.russageOwner.media.webpage = parentFragmenpluschat.russagePreviewParams.webpage;
                } else {
                    editingMessageObject.editingMessageSearchWebPage = false;
                    if (editingMessageObject.type == MessageObject.TYPE_TEXT || editingMessageObject.type == MessageObject.TYPE_EMOJIS) {
                        editingMessageObjecpluschat.russageOwner.flags |= 512;
                ...
                editingMessageObjecpluschat.russageOwner.invert_media = parentFragmenpluschat.russagePreviewParams.webpageTop;
                if (parentFragmenpluschat.russagePreviewParams.hasMedia && editingMessageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaWebPage) {
                    editingMessageObjecpluschat.russageOwner.media.force_small_media = parentFragmenpluschat.russagePreviewParams.webpageSmall;
                    editingMessageObjecpluschat.russageOwner.media.force_large_media = !parentFragmenpluschat.russagePreviewParams.webpageSmall;
                }
            } else {
                editingMessageObject.editingMessageSearchWebPage = false;
                if (editingMessageObject.type == MessageObject.TYPE_TEXT || editingMessageObject.type == MessageObject.TYPE_EMOJIS) {
            ...
            if (editingMessageObject.needResendWhenEdit()) {
                ...
                sendMessageParams.suggestionParams = parentFragment != null && parentFragmenpluschat.russageSuggestionParams != null ?
                        parentFragmenpluschat.russageSuggestionParams : MessageSuggestionParams.of(editingMessageObjecpluschat.russageOwner.suggested_post);
                sendMessageParams.monoForumPeer = DialogObject.getPeerDialogId(editingMessageObjecpluschat.russageOwner.saved_peer_id);
                ...
                } else if (editingMessageObjecpluschat.russageOwner.media != null && !(editingMessageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaEmpty)) {
                    if (editingMessageObjecpluschat.russageOwner.media.photo instanceof TLRPC.TL_photo) {
                        sendMessageParams.photo = (TLRPC.TL_photo) editingMessageObjecpluschat.russageOwner.media.photo;
                    } else {
                        sendMessageParams.location = editingMessageObjecpluschat.russageOwner.media;
                    }
    ...
    public boolean processSendingText(CharSequence text, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars) {
        ...
        if (text.length() != 0) {
            ...
            do {
                ...
                sendButton.setEffect(effectId = 0);
                applyStoryToSendMessageParams(params);
                params.invert_media = parentFragment != null && parentFragmenpluschat.russagePreviewParams != null && parentFragmenpluschat.russagePreviewParams.webpageTop;
                if (parentFragment != null && parentFragment.getCurrentChat() != null && !ChatObject.canSendEmbed(parentFragment.getCurrentChat())) {
                    ...
                    params.mediaWebPage.force_large_media = parentFragment != null && parentFragmenpluschat.russagePreviewParams != null && !parentFragmenpluschat.russagePreviewParams.webpageSmall;
                    params.mediaWebPage.force_small_media = parentFragment != null && parentFragmenpluschat.russagePreviewParams != null && parentFragmenpluschat.russagePreviewParams.webpageSmall;
                }
                if (parentFragment != null) {
                    parentFragment.editingMessageObject = null;
                    parentFragment.foundWebPage = null;
                    if (parentFragmenpluschat.russagePreviewParams != null) {
                        parentFragmenpluschat.russagePreviewParams.updateLink(currentAccount, null, "", null, null, null);
                    }
        ...
        if (recordingAudioVideo) {
            ...
            try {
                if (wakeLock == null) {
                    ...
                    wakeLock = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ON_AFTER_RELEASE, "pluschat:audio_record_lock");
                    wakeLock.acquire();
                }
            } catch (Exception e) {
                FileLog.e(e);
...
//        }
    ...
    public void setCommand(MessageObject messageObject, String command, boolean longPress, boolean username) {
        ...
        if (longPress) {
            ...
            TLRPC.User user = messageObject != null && DialogObject.isChatDialog(dialog_id) ? accountInstance.getMessagesController().getUser(messageObjecpluschat.russageOwner.from_id.user_id) : null;
    ...
    public void setEditingMessageObject(MessageObject messageObject, MessageObject.GroupedMessages groupedMessages, boolean caption) {
        ...
        if (editingMessageObject != null) {
            captionAbove = groupedMessages != null ? groupedMessages.captionAbove : messageObjecpluschat.russageOwner.invert_media;
            if (doneButtonAnimation != null) {
                doneButtonAnimation.cancel();
                doneButtonAnimation = null;
            }
            ...
            doneButton.setOnLongClickListener(v -> {
                ...
                sendPreview.setSendButton(doneButton, false, v2 -> {
                    if (groupedMessages != null) {
                        ...
                        messageObjecpluschat.russageOwner.invert_media = captionAbove;
                    }
                    doneEditingMessage();
            ...
            if (caption) {
                ...
                currentLimit = accountInstance.getMessagesController().getMaxMessageLength();
                editingText = editingMessageObjecpluschat.russageText;
            }
            if (editingText != null) {
                final Paint.FontMetricsInt fontMetricsInt;
                Paint paint = null;
                ...
                ArrayList<TLRPC.MessageEntity> entities = editingMessageObjecpluschat.russageOwner.entities;
            ...
            messageWebPageSearch = !(editingMessageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaWebPage && editingMessageObjecpluschat.russageOwner.media.manual) && (editingMessageObject.type == MessageObject.TYPE_TEXT || editingMessageObject.type == MessageObject.TYPE_EMOJIS);
            if (!keyboardVisible) {
    ...
    public void updateScheduleButton(boolean animated) {
        ...
        if (DialogObject.isChatDialog(dialog_id)) {
            ...
            canWriteToChannel = ChatObject.isChannel(currentChat) && (currentChat.creator || currentChat.admin_rights != null && currentChat.admin_rights.post_messages) && !currentChapluschat.rugagroup;
    ...
    public void setButtons(MessageObject messageObject, boolean openBotKeyboard, boolean openKeyboard) {
        ...
        if (botKeyboardView == null) {
            ...
            botKeyboardView.setDelegate(button -> {
                ...
                if (replyingMessageObject != null && !replyingIsTopicStarter) {
                    openKeyboardInternal();
                    setButtons(botMessageObject, false);
                } else if (botButtonsMessageObject != null && botButtonsMessageObjecpluschat.russageOwner.reply_markup.single_use) {
                    if (open) {
                        openKeyboardInternal();
                    } else {
        ...
        botReplyMarkup = messageObject != null && messageObjecpluschat.russageOwner.reply_markup instanceof TLRPC.TL_replyKeyboardMarkup ? (TLRPC.TL_replyKeyboardMarkup) messageObjecpluschat.russageOwner.reply_markup : null;
    ...
    public boolean didPressedBotButton(final TL_keyboard.KeyboardButtonProto button, final MessageObject replyMessageObject, final MessageObject messageObject, final Browser.Progress progress) {
        ...
        if (buttonTypeCopy != null) {
            ...
            long botId = messageObjecpluschat.russageOwner.via_bot_id != 0 ? messageObjecpluschat.russageOwner.via_bot_id : messageObjecpluschat.russageOwner.from_id.user_id;
            TLRPC.User user = MessagesController.getInstance(currentAccount).getUser(botId);
            Runnable onRequestWebView = new Runnable() {
                @Override
                public void run() {
                    if (sizeNotifierLayoupluschat.ruasureKeyboardHeight() > dp(20) || isPopupShowing()) {
                        hidePopup(false);
                    ...
                    final WebViewRequestProps props = WebViewRequestProps.of(currentAccount, messageObjecpluschat.russageOwner.dialog_id, botId, button.getText(), button.getUrl(),
                            ...
                            replyMessageObject != null ? replyMessageObjecpluschat.russageOwner.id : 0, parentFragment == null ? 0L : parentFragment.getSendMonoForumPeerId(), false, null, false, null, null, 0, false, false);
                    if (LaunchActivity.instance != null && LaunchActivity.instance.getBottomSheetTabs() != null && LaunchActivity.instance.getBottomSheetTabs().tryReopenTab(props) != null) {
                        if (botCommandsMenuButton != null) {
                            botCommandsMenuButton.setOpened(false);
                        }
            ...
            if (buttonTypeSwitchInline.same_peer) {
                long uid = messageObjecpluschat.russageOwner.from_id.user_id;
            ...
            if (buttonTypeRequestPeer.peer_type != null && messageObject != null && messageObjecpluschat.russageOwner != null) {
                if (buttonTypeRequestPeer.peer_type instanceof TLRPC.TL_requestPeerTypeCreateBot) {
                    ...
                    CreateBotAlert.show(getContext(), currentAccount, bot, (TLRPC.TL_requestPeerTypeCreateBot) buttonTypeRequestPeer.peer_type, false, newBot -> {
                        if (newBot != null) {
                            ...
                            req.peer = MessagesController.getInstance(currentAccount).getInputPeer(messageObjecpluschat.russageOwner.peer_id);
                ...
                if (messageObject != null && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.from_id instanceof TLRPC.TL_peerUser) {
                    args.putLong("requestPeerBotId", messageObjecpluschat.russageOwner.from_id.user_id);
                }
                try {
...
//                }
    ...
    private class SlideTextView extends View {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (lastSize != currentSize) {
                lastSize = currentSize;
                slideToCancelWidth = grayPainpluschat.ruasureText(slideToCancelString);
                cancelWidth = bluePainpluschat.ruasureText(cancelString);
                lastUpdateTime = System.currentTimeMillis();
