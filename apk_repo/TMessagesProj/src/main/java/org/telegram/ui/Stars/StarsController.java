...
import static org.pluschapluschat.russenger.MediaDataController.calcHash;
...
import org.pluschapluschat.russenger.AppGlobalConfig;
...
import org.pluschapluschat.russenger.FileRefController;
...
//        }
    ...
    public void openPaymentForm(MessageObject messageObject, TLRPC.InputInvoice inputInvoice, TLRPC.TL_payments_paymentFormStars form, Runnable whenShown, Utilities.Callback<String> whenAllDone) {
        ...
        final long dialogId = messageObject != null && messageObject.type == MessageObject.TYPE_PAID_MEDIA ? (
            (messageObjecpluschat.russageOwner.fwd_from != null && messageObjecpluschat.russageOwner.fwd_from.from_id != null) ?
                DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.fwd_from.from_id) :
    ...
    public void payAfterConfirmed(MessageObject messageObject, TLRPC.InputInvoice inputInvoice, TLRPC.TL_payments_paymentFormStars form, Utilities.Callback<Boolean> whenDone) {
        ...
        ConnectionsManager.getInstance(currentAccount).sendRequest(req2, (res2, err2) -> AndroidUtilities.runOnUIThread(() -> {
            ...
            if (res2 instanceof TLRPC.TL_payments_paymentResult) {
                ...
                final boolean media = messageObject != null && messageObjecpluschat.russageOwner != null && messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPaidMedia;
                if (media) {
    ...
    public class PendingPaidReactions {
        ...
        public void add(long amount, boolean affect) {
            ...
            if (affect) {
                ...
                minus += amount;
                NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.didUpdateReactions, messageObject.getDialogId(), messageObject.getId(), messageObjecpluschat.russageOwner.reactions);
                NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.starBalanceUpdated);
            } else {
                applied = false;
    ...
    public long getPendingPaidReactions(MessageObject messageObject) {
        if (messageObject == null || messageObjecpluschat.russageOwner == null) return 0;
        if ((messageObjecpluschat.russageOwner.isThreadMessage || messageObject.isForwardedChannelPost()) && messageObjecpluschat.russageOwner.fwd_from != null) {
            return getPendingPaidReactions(messageObject.getFromChatId(), messageObjecpluschat.russageOwner.fwd_from.saved_from_msg_id);
        } else {
...
//                    }
    ...
    private class PaidMessagesToast {
        ...
        public boolean pop(int messageId) {
            ...
            totalMessagesCount--;
            if (removedMessageObject != null && removedMessageObjecpluschat.russageOwner != null) {
                totalStars -= removedMessageObjecpluschat.russageOwner.paid_message_stars;
            }
