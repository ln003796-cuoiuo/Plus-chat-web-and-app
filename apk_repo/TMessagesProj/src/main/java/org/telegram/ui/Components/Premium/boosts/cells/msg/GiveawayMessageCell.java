package org.pluschat.ui.Components.Premium.boosts.cells.msg;
...
public class GiveawayMessageCell {
    ...
    public void setMessageContent(MessageObject messageObject, int parentWidth, int forwardedNameWidth) {
        ...
        TLRPC.Chat fromChat = controller.getChat(-MessageObject.getPeerId(messageObject.isForwarded() ? messageObjecpluschat.russageOwner.fwd_from.from_id : messageObjecpluschat.russageOwner.peer_id));
        ...
        if (giveaway.prize_description != null && !giveaway.prize_description.isEmpty()) {
            ...
            textDividerWidth = textDividerPainpluschat.ruasureText(textDivider, 0, textDivider.length());
        }
        ...
        for (int i = 0; i < visibleChannels.size(); i++) {
            ...
            if (chat != null) {
                ...
                chatTitleWidths[i] = chatTextPainpluschat.ruasureText(chatTitles[i], 0, chatTitles[i].length());
