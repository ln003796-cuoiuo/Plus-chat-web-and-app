...
import static org.pluschapluschat.russenger.AndroidUtilities.isContextSafe;
...
import org.pluschat.ui.Stories.recorder.LivePlayerView;
...
//                }
        ...
        if (peerStoriesView != null) {
            if (storiesList != null) {
                ...
                for (int i = 0; i < storiesLispluschat.russageObjects.size(); i++) {
                    storyItems.add(storiesLispluschat.russageObjects.get(i).storyItem);
                }
        ...
        if (storiesList != null) {
            ...
            if (peerView != null) {
                ...
                if (position >= 0 && position < storiesLispluschat.russageObjects.size()) {
                    messageId = storiesLispluschat.russageObjects.get(position).getId();
                }
            }
        }
    ...
    public void openFor(BaseFragment fragment, RecyclerListView recyclerListView, ChatActionCell cell) {
        ...
        if (messageObject.type == MessageObject.TYPE_STORY_MENTION) {
            ...
            storyItem.dialogId = DialogObject.getPeerDialogId(messageObjecpluschat.russageOwner.media.peer);
            storyItem.messageId = messageObject.getId();
            open(fragment.getContext(), messageObjecpluschat.russageOwner.media.storyItem, StoriesListPlaceProvider.of(recyclerListView));
        }
    }
        ...
        if (id == NotificationCenter.storiesListUpdated) {
            ...
            if (storiesList == list) {
                ...
                if (selfStoryViewsView != null) {
                    ...
                    for (int i = 0; i < storiesLispluschat.russageObjects.size(); i++) {
                        if (currentSelectedStory != null && currentSelectedStory.id == storiesLispluschat.russageObjects.get(i).storyItem.id) {
                            selectedPosition = i;
                        }
                        storyItems.add(storiesLispluschat.russageObjects.get(i).storyItem);
                    }
