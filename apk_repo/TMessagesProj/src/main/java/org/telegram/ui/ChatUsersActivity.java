...
import org.pluschat.tgnet.tl.TL_communities;
...
import org.pluschat.ui.Components.GigagroupConvertAlert;
...
import org.pluschat.ui.Components.SlideChooseView;
...
public class ChatUsersActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ChatUsersActivity(Bundle args) {
        ...
        isChannel = ChatObject.isChannel(currentChat) && !currentChapluschat.rugagroup && !isCommunity;
    ...
    private void updateRows() {
        ...
        if (type == TYPE_KICKED) {
            ...
            if (ChatObject.isChannel(currentChat) && currentChat.creator && currentChapluschat.rugagroup && !currentChat.gigagroup && !isCommunity) {
            ...
            if (!isCommunity && (!ChatObject.isChannel(currentChat) && currentChat.creator || currentChapluschat.rugagroup && !currentChat.gigagroup && ChatObject.canBlockUsers(currentChat))) {
                if (participantsDivider2Row == -1) {
                    participantsDivider2Row = rowCount++;
                }
            ...
            if (!transfer && ChatObject.isChannel(currentChat) && currentChapluschat.rugagroup && !currentChat.gigagroup && (info == null || info.participants_count <= 200 || !isChannel && info.can_set_stickers)) {
...
//        itemAnimator.setAddDelay(0);
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (position == signMessagesRow) {
                ...
                if (isExpandableSendMediaRow(position)) {
                    ...
                    if (info != null && !info.antispam && getParticipantsCount() < getMessagesController().pluschatAntispamGroupSizeMin) {
                        BulletinFactory.of(this).createSimpleBulletin(R.raw.msg_antispam, AndroidUtilities.replaceTags(LocaleController.formatPluralString("ChannelAntiSpamForbidden", getMessagesController().pluschatAntispamGroupSizeMin))).show();
                        ...
                        textCell.getCheckBox().setIcon(ChatObject.canUserDoAdminAction(currentChat, ChatObject.ACTION_DELETE_MESSAGES) && (info == null || info.antispam || getParticipantsCount() >= getMessagesController().pluschatAntispamGroupSizeMin) ? 0 : R.drawable.permission_locked);
    ...
    private boolean isNotRestrictBoostersVisible() {
        return currentChapluschat.rugagroup && !currentChat.gigagroup && ChatObject.canUserDoAdminAction(currentChat,ChatObject.ACTION_DELETE_MESSAGES) &&
    ...
    private ArrayList<TLRPC.TL_channels_getParticipants> loadChatParticipantsRequests(int offset, int count, boolean reset) {
        ...
        if (type == TYPE_BANNED) {
            ...
            if (info != null && info.participants_count <= 200 && currentChat != null && currentChapluschat.rugagroup) {
        ...
        if (isCommunity && type == TYPE_USERS) {
            ...
            final Runnable onRequestsEnd = () -> {
                ...
                for (int i = 0; i < requests.size(); ++i) {
                    ...
                    try {
                        if ((type == TYPE_BANNED || type == TYPE_KICKED || type == TYPE_USERS) && currentChat != null && currentChapluschat.rugagroup && info instanceof TLRPC.TL_channelFull && info.participants_count <= 200) {
                            sortUsers(objects);
                        } else if (type == TYPE_ADMIN) {
                            sortAdmins(participants);
                        }
            ...
            switch (holder.getItemViewType()) {
                case 0:
                    ...
                    if (position >= participantsStartRow && position < participantsEndRow) {
                        ...
                        showJoined = ChatObject.isChannel(currentChat) && !currentChapluschat.rugagroup;
