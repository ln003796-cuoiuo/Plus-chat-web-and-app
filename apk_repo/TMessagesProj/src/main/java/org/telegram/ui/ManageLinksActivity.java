...
import org.pluschat.ui.Cells.CreationTextCell;
...
import org.pluschat.ui.Components.LinkActionView;
...
public class ManageLinksActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
            ...
            switch (holder.getItemViewType()) {
                case 2:
                    ...
                    if (isPublic && adminId == getAccountInstance().getUserConfig().clientUserId) {
                        if (info != null) {
                            linkActionView.setLink("https://pluschat.ru/" + ChatObject.getPublicUsername(currentChat));
    ...
    private class LinkCell extends FrameLayout {
        ...
        public void setLink(TLRPC.TL_chatInviteExported invite, int position) {
            ...
            } else if (invite.link.startsWith("https://pluschat.ru/+")) {
                titleView.setText(MessagesController.getInstance(currentAccount).linkPrefix + "/" + invite.link.substring("https://pluschat.ru/+".length()));
            } else if (invite.link.startsWith("https://pluschat.ru/joinchat/")) {
                titleView.setText(invite.link.substring("https://pluschat.ru/joinchat/".length()));
