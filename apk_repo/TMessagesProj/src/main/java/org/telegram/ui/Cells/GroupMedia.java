...
import org.pluschapluschat.russenger.MessageObject.GroupedMessagePosition;
...
public class GroupMedia {
    ...
    public void setMessageObject(MessageObject messageObject, boolean pinnedBottom, boolean pinnedTop) {
        ...
        if (!(messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPaidMedia)) return;

        final TLRPC.TL_messageMediaPaidMedia paidMedia = (TLRPC.TL_messageMediaPaidMedia) messageObjecpluschat.russageOwner.media;

        if (layout == null) {
            layout = new GroupedMessages();
        }
        layoupluschat.rudias.clear();
        layoupluschat.rudias.addAll(paidMedia.extended_media);
        layout.calculate();
        ...
        for (int i = 0; i < paidMedia.extended_media.size(); ++i) {
            ...
            if (holder == null) {
                ...
                if (media.attachPath != null) {
                    ...
                    holder.attachPath = messageObjecpluschat.russageOwner != null ? messageObjecpluschat.russageOwner.attachPath : null;
                }
...
//        }
    ...
    public void updateHolders(MessageObject messageObject) {
        ...
        if (hidden) {
            final TLRPC.TL_messageMediaPaidMedia paidMedia = messageObject == null ? null : (TLRPC.TL_messageMediaPaidMedia) messageObjecpluschat.russageOwner.media;
            if (paidMedia != null) {
                buttonText = new Text(StarsIntroActivity.replaceStarsWithPlain(LocaleController.formatPluralStringComma("UnlockPaidContent", (int) (buttonTextPrice = paidMedia.stars_amount)), .7f), 14, AndroidUtilities.bold());
                if (buttonText.getCurrentWidth() > width - dp(30)) {
                    buttonText = new Text(StarsIntroActivity.replaceStarsWithPlain(LocaleController.formatPluralStringComma("UnlockPaidContentShort", (int) (buttonTextPrice = paidMedia.stars_amount)), .7f), 14, AndroidUtilities.bold());
    ...
    public ImageReceiver getPhotoImage(int index) {
        ...
        if (index < 0 || index >= layoupluschat.rudias.size()) return null;
        TLRPC.MessageExtendedMedia media = layoupluschat.rudias.get(index);
        for (int i = 0; i < holders.size(); ++i) {
