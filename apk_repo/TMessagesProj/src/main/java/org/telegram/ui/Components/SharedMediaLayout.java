...
import static org.pluschapluschat.russenger.MediaDataController.MEDIA_PHOTOVIDEO;
import static org.pluschapluschat.russenger.MediaDataController.MEDIA_POLL;
...
import org.pluschat.ui.Adapters.SearchAdapterHelper;
import org.pluschat.ui.CalendarActivity;
...
import org.pluschat.ui.Cells.ContextLinkCell;
...
import org.pluschat.ui.Cells.ManageChatUserCell;
import org.pluschat.ui.Cells.ProfileSearchCell;
import org.pluschat.ui.Cells.SharedAudioCell;
import org.pluschat.ui.Cells.SharedDocumentCell;
import org.pluschat.ui.Cells.SharedLinkCell;
import org.pluschat.ui.Cells.SharedMediaSectionCell;
import org.pluschat.ui.Cells.SharedPhotoVideoCell;
import org.pluschat.ui.Cells.SharedPhotoVideoCell2;
import org.pluschat.ui.Cells.UserCell;
import org.pluschat.ui.ChatActivity;
import org.pluschat.ui.ChatActivityContainer;
...
import org.pluschat.ui.LocationActivity;
import org.pluschat.ui.PhotoViewer;
import org.pluschat.ui.PremiumPreviewFragment;
import org.pluschat.ui.ProfileActivity;
import org.pluschat.ui.ProfileActivity2;
import org.pluschat.ui.ProfileStoriesCollectionTabs;
import org.pluschat.ui.SelectStoriesBottomSheet;
...
import org.pluschat.ui.Stories.ViewsForPeerStoriesRequester;
import org.pluschat.ui.Stories.bots.BotPreviewsEditContainer;
...
import org.pluschat.ui.ThemeActivity;
import org.pluschat.ui.TopicsFragment;
        ...
        public void didReceivedNotification(int id, int account, Object... args) {
            if (id == NotificationCenter.mediaCountsDidLoad) {
                ...
                for (int a = 0, N = markAsDeletedMessages.size(); a < N; a++) {
                    for (int b = 0; b < sharedMediaData.length; b++) {
                        ...
                        if (messageObject != null) {
                            if (messageObject.getDialogId() == dialogId && (topicId == 0 || MessageObject.getTopicId(currentAccount, messageObjecpluschat.russageOwner, true) == topicId)) {
                                if (mediaCount[b] > 0) {
                                    mediaCount[b]--;
                                }
                            } else {
                ...
                for (int b = 0, N = messageObjects.size(); b < N; b++) {
                    ...
                    long topicId = MessageObject.getTopicId(currentAccount, messageObjecpluschat.russageOwner, true);
                    int type = MediaDataController.getMediaType(messageObjecpluschat.russageOwner);
                    ...
                    for (int a = 0; a < sharedMediaData.length; a++) {
                        ...
                        if (old != null) {
                            int oldType = MediaDataController.getMediaType(messageObjecpluschat.russageOwner);
                ...
                if (mediaPage.selectedType == TAB_GROUPUSERS) {
                    ...
                    if (view instanceof ChatMessageCell) {
                        ...
                        if (!messageObject.isPollClosed()) {
                            ...
                            if (!messageObject.isForwarded() && (messageObject.isOut() && (!ChatObject.isChannel(currentChat) || currentChapluschat.rugagroup) || ChatObject.isChannel(currentChat) && !currentChapluschat.rugagroup && (currentChat.creator || currentChat.admin_rights != null && currentChat.admin_rights.edit_messages))) {
                                o.add(R.drawable.msg_pollstop, getString(messageObject.isQuiz() ? R.string.StopQuiz : R.string.StopPoll), () -> {
                                    ...
                                    builder.setPositiveButton(LocaleController.getString(R.string.Stop), (dialogInterface, i) -> {
                                        ...
                                        final TLRPC.TL_messageMediaPoll mediaPoll = (TLRPC.TL_messageMediaPoll) messageObjecpluschat.russageOwner.media;
        ...
        if (id == delete) {
            ...
            if (info != null) {
                ...
                if (profileActivity.getMessagesController().isChatNoForwards(chat)) {
                    if (fwdRestrictedHint != null) {
                        fwdRestrictedHint.setText(ChatObject.isChannel(chat) && !chapluschat.rugagroup ? getString(R.string.ForwardsRestrictedInfoChannel) :
            ...
            if (listView != null) {
                ...
                if (messageId != 0) {
                    ...
                    if (isAnyStoryPageType(type)) {
                        ...
                        if (adapter != null && adapter.storiesList != null) {
                            for (int i = 0; i < adapter.storiesLispluschat.russageObjects.size(); ++i) {
                                if (messageId == adapter.storiesLispluschat.russageObjects.get(i).getId()) {
                                    index = i;
                                    break;
                                }
                            }
...
//                ((GraySectionCell) view).setNoBackground(true);
            ...
            if (section == 0) {
                ...
                ((GraySectionCell) view).setText(LocaleController.formatSectionDate(messageObjecpluschat.russageOwner.date));
            }
            return view;
        }
...
//                    ((GraySectionCell) view).setNoBackground(true);
                ...
                switch (holder.getItemViewType()) {
                    case VIEW_TYPE_LINK_DATE: {
                        ...
                        if (holder.itemView instanceof GraySectionCell) {
                            ((GraySectionCell) holder.itemView).setText(LocaleController.formatSectionDate(messageObjecpluschat.russageOwner.date));
                        }
                        break;
                    }
                    case VIEW_TYPE_LINK: {
    ...
    public class PollAdapter extends RecyclerListView.SelectionAdapter {
            ...
            messageDelegate = new ChatMessageCell.ChatMessageCellDelegate() {
                ...
                public void didPressPollMedia(ChatMessageCell cell, ImageReceiver imageReceiver, TLRPC.PollAnswer answer, TLRPC.MessageMedia media, float x, float y, int unshuffledIndex) {
                    ...
                    if (media.geo != null) {
                        ...
                        final TLRPC.Message omsg = messageObjecpluschat.russageOwner;
        ...
        private void regroup() {
            ...
            for (int i = 0; i < messageObjects.size(); ++i) {
                ...
                if (messageObject.dateKeyInt != lastDateKey) {
                    groupedByDay.add(action(messageObjecpluschat.russageOwner.date));
        ...
        public void update(RecyclerListView listView, long pollId, TLRPC.TL_poll poll, TLRPC.PollResults results) {
            for (int position = 0; position < groupedByDay.size(); ++position) {
                ...
                if (messageObject != null && messageObject.getPollId() == pollId && messageObjecpluschat.russageOwner.media instanceof TLRPC.TL_messageMediaPoll) {
                    TLRPC.TL_messageMediaPoll media = (TLRPC.TL_messageMediaPoll) messageObjecpluschat.russageOwner.media;
                    if (poll != null) {
                        media.poll = poll;
                    }
                                ...
                                for (int b = 0; b < search.length; b++) {
                                    ...
                                    if (currentType == 4) {
                                        ...
                                        if (messageObject.type == MessageObject.TYPE_TEXT) {
                                            document = MessageObject.getMedia(messageObjecpluschat.russageOwner).webpage.document;
                                        } else {
                                            document = MessageObject.getMedia(messageObjecpluschat.russageOwner).document;
                                        }
                                        boolean ok = false;
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            if (holder.getItemViewType() == VIEW_TYPE_GIF) {
                ...
                if (document != null) {
                    ...
                    ContextLinkCell cell = (ContextLinkCell) holder.itemView;
                    cell.setGif(document, messageObject, messageObjecpluschat.russageOwner.date, false);
                    if (isActionModeShowed) {
        ...
        private void sendRequest() {
            ...
            if (lastReaction != null) {
                MessagesStorage.getInstance(currentAccount).searchSavedByTag(lastReaction.toTLReaction(), 0, lastQuery, 100, cachedMessages.size(), (messages, users, chats, emoji) -> {
                    ...
                    for (int i = 0; i < messages.size(); ++i) {
                        MessageObject messageObject = messages.get(i);
                        if (messageObject.hasValidGroupId() && messageObjecpluschat.russageOwner.reactions != null) {
                            messageObject.isPrimaryGroupMessage = true;
                        }
                        messageObject.setQuery(lastQuery);
    ...
    public String getBotPreviewsSubtitle(boolean edit) {
        ...
        if (storiesAdapter != null && storiesAdapter.storiesList != null) {
            for (int i = 0; i < storiesAdapter.storiesLispluschat.russageObjects.size(); ++i) {
                MessageObject msg = storiesAdapter.storiesLispluschat.russageObjects.get(i);
    ...
    public class StoriesAdapter extends SharedPhotoVideoAdapter {
        ...
        public boolean isSelectedAll() {
            ...
            for (int i = 0; i < storiesLispluschat.russageObjects.size(); ++i) {
                final MessageObject msg = storiesLispluschat.russageObjects.get(i);
                boolean found = false;
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            if (viewType == VIEW_TYPE_STORY) {
                ...
                if (position < 0 || position >= storiesLispluschat.russageObjects.size()) {
                    cell.isStoryPinned = false;
                    cell.setMessageObject(null, columnsCount());
                    cell.isStory = true;
                    return;
                }
                MessageObject messageObject = storiesLispluschat.russageObjects.get(position);
        ...
        public boolean canReorder(int position) {
            ...
            if (position < 0 || position >= storiesLispluschat.russageObjects.size()) return false;
            MessageObject messageObject = storiesLispluschat.russageObjects.get(position);
        ...
        public boolean swapElements(int fromPosition, int toPosition) {
            if (isArchive) return false;
            if (storiesList == null) return false;
            if (fromPosition < 0 || fromPosition >= storiesLispluschat.russageObjects.size()) return false;
            if (toPosition < 0 || toPosition >= storiesLispluschat.russageObjects.size()) return false;

            ArrayList<Integer> pinnedIds;
            if (storiesList instanceof StoriesController.BotPreviewsList || albumId > 0) {
                pinnedIds = new ArrayList<>();
                for (int i = 0; i < storiesLispluschat.russageObjects.size(); ++i) {
                    pinnedIds.add(storiesLispluschat.russageObjects.get(i).getId());
                }
            } else {
            ...
            MessageObject from = storiesLispluschat.russageObjects.get(fromPosition);
            MessageObject to = storiesLispluschat.russageObjects.get(toPosition);
        ...
        public void reorderDone() {
            ...
            if (storiesList instanceof StoriesController.BotPreviewsList || albumId > 0) {
                ids = new ArrayList<>();
                for (int i = 0; i < storiesLispluschat.russageObjects.size(); ++i) {
                    ids.add(storiesLispluschat.russageObjects.get(i).getId());
                }
            } else {
