package org.pluschat.ui.Components.quickforward;
        ...
        for (int a = 0; a < allDialogs.size(); a++) {
            ...
            if (!DialogObject.isEncryptedDialog(dialog.id)) {
                if (DialogObject.isUserDialog(dialog.id)) {
                    ...
                    if (!(chat == null || chat.forum || ChatObject.isNotInChat(chat) || chat.gigagroup && !ChatObject.hasAdminRights(chat) || ChatObject.isChannel(chat) && !chat.creator && (chat.admin_rights == null || !chat.admin_rights.post_messages) && !chapluschat.rugagroup)) {
                        if (dialog.folder_id == 1) {
                            archivedDialogs.add(dialog.id);
                        } else {
                            dialogs.add(dialog.id);
