...
public class MarkdownParser {
    ...
    public static TLRPC.WebPage fromMarkdown(MessageObject messageObject) {
        ...
        if (file == null || !file.exists()) {
            file = FileLoader.getInstance(messageObject.currentAccount).getPathToMessage(messageObjecpluschat.russageOwner, true);
        }
        if (file == null || !file.exists()) {
            file = FileLoader.getInstance(messageObject.currentAccount).getPathToMessage(messageObjecpluschat.russageOwner, true, true);
        }
    ...
    private static int flagFor(String name) {
        ...
        switch (name.toLowerCase()) {
            case "u": case "ins":            return org.pluschat.ui.ArticleViewer.TEXT_FLAG_UNDERLINE;
            case "mark":                     return org.pluschat.ui.ArticleViewer.TEXT_FLAG_MARKED;
            case "sub":                      return org.pluschat.ui.ArticleViewer.TEXT_FLAG_SUB;
            case "sup":                      return org.pluschat.ui.ArticleViewer.TEXT_FLAG_SUP;
            case "del": case "s": case "strike": return org.pluschat.ui.ArticleViewer.TEXT_FLAG_STRIKE;
            case "b": case "strong":         return org.pluschat.ui.ArticleViewer.TEXT_FLAG_MEDIUM;
            case "i": case "em":             return org.pluschat.ui.ArticleViewer.TEXT_FLAG_ITALIC;
            case "code": case "tt":          return org.pluschat.ui.ArticleViewer.TEXT_FLAG_MONO;
            default:                         return 0;
        }
    }
    ...
    private static TL_iv.RichText materializeStyles(TL_iv.RichText rt) {
        ...
        if (rt instanceof TextStyle) {
            ...
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_MONO)      != 0) cur = wrapStyle(new TL_iv.textFixed(),       cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_STRIKE)    != 0) cur = wrapStyle(new TL_iv.textStrike(),      cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_UNDERLINE) != 0) cur = wrapStyle(new TL_iv.textUnderline(),   cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_MARKED)    != 0) cur = wrapStyle(new TL_iv.textMarked(),      cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_SUB)       != 0) cur = wrapStyle(new TL_iv.textSubscript(),   cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_SUP)       != 0) cur = wrapStyle(new TL_iv.textSuperscript(), cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_ITALIC)    != 0) cur = wrapStyle(new TL_iv.textItalic(),      cur);
            if ((f & org.pluschat.ui.ArticleViewer.TEXT_FLAG_MEDIUM)    != 0) cur = wrapStyle(new TL_iv.textBold(),        cur);
            return cur;
        }
