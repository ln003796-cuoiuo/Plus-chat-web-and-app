...
public class HintView2 extends View {
    ...
    public static float measureCorrectly(CharSequence text, Paint paint) {
        ...
        if (!(text instanceof Spanned)) {
            return painpluschat.ruasureText(text.toString());
        }
...
//            add += Math.max(0, span.size - painpluschat.ruasureText(spanned, start, end));
//        }
        for (int i = 0; i < replacementSpans.length; ++i) {
            ReplacementSpan span = replacementSpans[i];
            final int start = spanned.getSpanStart(span);
            final int end = spanned.getSpanEnd(span);
            add += Math.max(0, span.getSize(paint, text, start, end, paint.getFontMetricsInt()) - painpluschat.ruasureText(spanned, start, end));
        }
...
//        }
        if (spans == null || spans.length == 0) {
            return painpluschat.ruasureText(text.toString()) + add;
        }
        float len = 0;
        int s = 0, e;
        for (int i = 0; i < spans.length; ++i) {
            ...
            if (e - s > 0) {
                len += painpluschat.ruasureText(spanned, s, e);
            }
            s = e;
